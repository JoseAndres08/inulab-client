...
async function cargarPreciosDesdeAPI() {
  try {
    ...
    const response = await fetch(`${API_BASE}/Precios`, {
      headers
    });
...
const PdfViewer = ({
  ...
  React.useEffect(() => {
    ...
    fetch(url, {
      ...
      return r.blob();
    }).then(blob => setBlobUrl(URL.createObjectURL(blob))).catch(() => setError(true));
  }, [url]);
...
const App = () => {
  ...
  const [pedidosSearchTerm, setPedidosSearchTerm] = useState(''); // Estados búsqueda dueño
  ...
  const renderTermsModal = () => showTermsModal && /*#__PURE__*/React.createElement("div", {
  ...
  }, freshUser?.type === 'dueño' && /*#__PURE__*/React.createElement("div", {
    className: "mb-8"
  }, /*#__PURE__*/React.createElement("div", {
    className: "mb-4"
  }, /*#__PURE__*/React.createElement("div", {
    ...
    className: "space-y-3"
  }, termsContent.tomaMuestra.map((item, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    ...
    className: "space-y-3"
  }, termsContent.notas.map((nota, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    className: "flex gap-3"
  }, /*#__PURE__*/React.createElement("span", {
    ...
    className: "space-y-3"
  }, termsContent.tenerEnCuenta.map((item, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    className: "flex gap-3"
  }, /*#__PURE__*/React.createElement("span", {
  ...
  const renderTermsCheckbox = () => /*#__PURE__*/React.createElement("div", {
    className: "mb-4"
  ...
  const [facturacionFilterYear, setFacturacionFilterYear] = useState(''); // DEMO: Funciones de persistencia localStorage
  ...
  const getCurrentNightMode = () => {
    try {
      const s = localStorage.getItem('inulab_db_v25');
      if (s) return JSON.parse(s).nightMode;
    } catch (e) {}
    return database?.nightMode || false;
  ...
  const TrackingMapComponent = ({
    ...
    const routePoints = [[-12.0564, -77.0528], [-12.0544, -77.0508], [-12.0524, -77.0488], [-12.0504, -77.0468], [-12.0484, -77.0448], [-12.0464, -77.0428]];
    ...
    useEffect(() => {
      if (mapRef.current && !mapInstanceRef.current) {
        ...
        const motoMarker = L.marker(routePoints[0], {
          icon: L.divIcon({
  ...
  const [pendingNewInvoice, setPendingNewInvoice] = useState(false); // Efecto para detectar nuevos resultados y comprobantes (solo marca como pendiente)
  useEffect(() => {
  ...
  const loadDatabase = async (silent = false) => {
    ...
    try {
      ...
      const addresses = Array.isArray(addressesResponse) ? addressesResponse : addressesResponse?.$values || addressesResponse?.addresses || addressesResponse?.data || []; // Enriquecer exams con pdfData de orders
      pets.forEach(pet => {
        ...
        pet.exams = pet.exams.map(exam => {
          console.log('EXAM TYPE:', exam.type, '| ITEMS:', orders.map(o => o.items?.map(i => i.examName)));
          const matchingOrder = orders.find(o => (o.status === 'completed' || Number(o.status) === 9) && (o.items || []).some(i => String(i.examName).toLowerCase() === String(exam.type || exam).toLowerCase()));
          const rawUrl = matchingOrder?.resultPdfUrl || null;
          const pdfUrl = rawUrl ? rawUrl.startsWith('http') ? rawUrl : `https://inulab-backend-production.up.railway.app${rawUrl}` : null;
          return {
            ...exam,
            pdfData: pdfUrl
          };
  ...
  const saveProfile = async (changingName = false) => {
    ...
    try {
      saveDatabase(prev => ({
        ...prev,
        users: prev.users.map((u, i) => i === userIndex ? user : u)
      }));
      setCurrentUser(user);
    } catch (err) {
      alert(err.message);
  ...
  const removeFromPending = examId => {
    setPendingExams(pendingExams.filter(e => e.id !== examId));
    if (currentExamForPet?.id === examId) setCurrentExamForPet(null);
  };
  const removeFromCart = examId => {
    setCart(cart.filter(c => c.exam.id !== examId));
  };
  ...
  const addNewPet = async () => {
    ...
    try {
      ...
      const response = await api.createPet(petData); // Algunas APIs devuelven { pet: {...} } y otras directamente el objeto
      const pet = response?.data || response?.pet || response; // Agregar al estado del frontend
      setDatabase(prev => ({
        ...prev,
    ...
    if (exam.seen === false) {
      try {
        saveDatabase(prev => {
          ...
          const updatedExams = (user[petsArray][petIndex].exams || []).map(e => e.id === exam.id ? {
            ...e,
            seen: true
          } : e);
          const updatedPets = (user[petsArray] || []).map((p, i) => i === petIndex ? {
            ...p,
            exams: updatedExams
          } : p);
          return {
            ...prev,
            users: (prev.users || []).map((u, i) => i === userIndex ? {
              ...u,
              [petsArray]: updatedPets
            } : u),
  ...
  const filteredCategorias = searchTerm.trim() === '' ? categorias : categorias.map(cat => ({
    ...cat,
    examenes: cat.examenes.filter(e => flexibleSearch(e.name, searchTerm) || e.subtitle && flexibleSearch(e.subtitle, searchTerm))
  })).filter(cat => cat.examenes.length > 0);
  const hayResultados = filteredPerfiles.length > 0 || filteredCategorias.length > 0; // LOGIN PRIMERO
  if (!isAuthenticated) {
  ...
  const freshUser = currentUser; // Vista PDF (solo móvil o usuarios no médicos)
  ...
  if (viewingInvoice && freshUser.type !== 'medico' && window.innerWidth < 768) {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: `fas ${viewingInvoice.documentType === 'boleta' ? 'fa-receipt' : 'fa-file-invoice'} text-5xl mb-3 text-gray-300`
    }), /*#__PURE__*/React.createElement("p", {
      className: "text-sm"
  ...
  if (selectedExam && freshUser.type === 'medico' && window.innerWidth < 768) {
    return /*#__PURE__*/React.createElement("div", {
      ...
      onClick: () => {
        ...
        link.download = `${selectedExam.type.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
      },
  ...
  const petsOrPatients = freshUser.type === 'medico' ? database?.pets || [] : database?.pets || [];
  const entityName = freshUser.type === 'medico' ? 'paciente' : 'mascota';
  const filteredPets = petSearchTerm.trim() === '' ? database?.pets || [] : (database?.pets || []).filter(p => p.name.toLowerCase().includes(petSearchTerm.toLowerCase()));
  const resetDuenoState = () => {
    setSelectedPet(null);
    setDuenoView('main');
    setShowEditProfile(false);
  ...
  const renderDuenoHeader = () => /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    ...
    onClick: () => cart.length > 0 && setShowOrderSummary(true),
    className: `relative w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${cart.length > 0 ? 'bg-cyan-100 text-cyan-700' : 'bg-gray-100 text-gray-300 cursor-default'}`
  }, /*#__PURE__*/React.createElement("i", {
    ...
    onClick: () => setDuenoMenuOpen(!duenoMenuOpen),
    ...
    onClick: () => setDuenoMenuOpen(false)
  }, /*#__PURE__*/React.createElement("div", {
  ...
  const renderDuenoSidebar = () => /*#__PURE__*/React.createElement("div", {
    ...
    className: `w-8 h-8 rounded-lg flex items-center justify-center ${activeTab === 'pedidos' && duenoView !== 'config' ? 'bg-white/20' : 'bg-cyan-100'}`
  }, /*#__PURE__*/React.createElement("i", {
    className: `fas fa-clipboard-list ${activeTab === 'pedidos' && duenoView !== 'config' ? 'text-white' : 'text-cyan-600'}`
  })), /*#__PURE__*/React.createElement("span", {
    className: "font-medium"
    ...
    className: `w-8 h-8 rounded-lg flex items-center justify-center ${activeTab === 'resultados' && duenoView !== 'config' ? 'bg-white/20' : 'bg-purple-100'}`
  }, /*#__PURE__*/React.createElement("i", {
    className: `fas fa-file-medical ${activeTab === 'resultados' && duenoView !== 'config' ? 'text-white' : 'text-purple-600'}`
  })), /*#__PURE__*/React.createElement("span", {
    className: "font-medium"
  }, "Resultados"), countUnseenExams(petsOrPatients) + countPendingPayments() > 0 && /*#__PURE__*/React.createElement("span", {
    className: `ml-auto text-xs px-2 py-0.5 rounded-full ${activeTab === 'resultados' && duenoView !== 'config' ? 'bg-white/20' : 'bg-red-100 text-red-600'}`
  }, countUnseenExams(petsOrPatients) + countPendingPayments())), /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      resetDuenoState();
    ...
    className: `w-8 h-8 rounded-lg flex items-center justify-center ${activeTab === 'facturacion' ? 'bg-amber-400' : 'bg-amber-100'}`
  }, /*#__PURE__*/React.createElement("i", {
    className: `fas fa-file-invoice-dollar text-sm ${activeTab === 'facturacion' ? 'text-white' : 'text-amber-600'}`
  })), /*#__PURE__*/React.createElement("span", {
    className: "font-medium"
  ...
  }), "WhatsApp Inulab"))); // Vista de Tracking de Pedido (para clientes)
  ...
  if (viewingOrderTracking && window.innerWidth < 768) {
    ...
    const currentIndex = statusOrder.indexOf(order.status === 'pending' ? 'confirmed' : order.status); // Solo mostrar esta vista en mobile, en desktop se maneja dentro del layout del médico
    if (userType === 'medico') {// Para médicos, no retornamos aquí - dejamos que el código del médico maneje el tracking
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        onClick: () => setViewingOrderTracking(null),
        ...
        return /*#__PURE__*/React.createElement("div", {
          ...
          className: `w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${isCompleted ? 'bg-cyan-500 text-white' : 'bg-gray-200 text-gray-400'}`
        }, isCompleted ? /*#__PURE__*/React.createElement("i", {
          className: "fas fa-check text-xs"
        }) : /*#__PURE__*/React.createElement("i", {
          className: `fas ${step.icon} text-xs`
        })), !isLast && /*#__PURE__*/React.createElement("div", {
          className: `w-0.5 h-10 ${index < currentIndex ? 'bg-cyan-500' : 'bg-gray-200'}`
        })), /*#__PURE__*/React.createElement("div", {
          className: `flex-1 flex items-start justify-between ${!isLast ? 'pb-3' : ''}`
        }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("p", {
          className: `font-medium text-sm ${isCompleted ? 'text-gray-800' : 'text-gray-400'}`
        ...
        className: `w-10 h-10 ${item?.exam?.bg || 'bg-cyan-100'} rounded-lg flex items-center justify-center`
      }, /*#__PURE__*/React.createElement("i", {
        className: `fas ${item?.exam?.icon || 'fa-vial'} ${item?.exam?.color || 'text-cyan-600'}`
      })), /*#__PURE__*/React.createElement("div", {
        className: "flex-1"
      }, /*#__PURE__*/React.createElement("p", {
        className: "font-medium text-gray-800 text-sm"
      }, item?.exam?.name || item?.examName || 'Examen'), /*#__PURE__*/React.createElement("p", {
        className: "text-gray-500 text-xs"
      }, item?.pet?.name || ''))))), /*#__PURE__*/React.createElement("div", {
      ...
      }, addr ? `${addr.street}, ${addr.district}` : 'Sin dirección')))), order.comment && /*#__PURE__*/React.createElement("div", {
        className: "mt-3 pt-3 border-t border-gray-100"
      }, /*#__PURE__*/React.createElement("div", {
        className: "flex items-start gap-2 text-sm"
      }, /*#__PURE__*/React.createElement("i", {
  ...
  if (selectedPet && activeTab === 'resultados' && freshUser.type !== 'dueño') {
    ...
    const petProfileContent = /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("button", {
      ...
      className: "space-y-3"
    }, (currentPet?.exams || []).map(exam => /*#__PURE__*/React.createElement("div", {
      key: exam.id,
      onClick: () => markExamAsSeen(exam, currentPet),
      ...
      className: `w-10 h-10 rounded-xl flex items-center justify-center ${exam.seen === false ? 'bg-red-100' : 'bg-cyan-100'}`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas fa-file-pdf ${exam.seen === false ? 'text-red-600' : 'text-cyan-600'}`
    })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("p", {
  ...
  if (currentExamForPet && freshUser.type !== 'medico' && freshUser.type !== 'dueño') {
    ...
    const selectionBody = /*#__PURE__*/React.createElement("div", {
      className: "p-4 max-w-lg mx-auto"
    }, /*#__PURE__*/React.createElement("div", {
      className: "bg-white rounded-2xl shadow p-4 mb-4 flex items-center gap-4"
    }, /*#__PURE__*/React.createElement("div", {
      className: `w-12 h-12 ${currentExamForPet.bg} rounded-xl flex items-center justify-center`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas ${currentExamForPet.icon} ${currentExamForPet.color} text-xl`
    })), /*#__PURE__*/React.createElement("div", {
      className: "flex-1"
    }, /*#__PURE__*/React.createElement("p", {
      className: "font-bold text-gray-800"
    ...
    }, showAddressSelection ? `${selectedPetForOrder?.name} - Selecciona dirección` : `Selecciona ${entityName}`))), pendingExams.length > 1 && /*#__PURE__*/React.createElement("div", {
      className: "bg-blue-50 rounded-xl p-3 mb-4 text-center"
    }, /*#__PURE__*/React.createElement("p", {
      className: "text-sm text-blue-700"
    }, "Examen ", pendingExams.findIndex(e => e.id === currentExamForPet.id) + 1, " de ", pendingExams.length)), /*#__PURE__*/React.createElement("div", {
      className: "flex items-center justify-center gap-2 mb-4"
    }, /*#__PURE__*/React.createElement("div", {
      className: `flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium ${!showAddressSelection ? 'bg-cyan-500 text-white' : 'bg-emerald-100 text-emerald-700'}`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas ${!showAddressSelection ? 'fa-paw' : 'fa-check'}`
    }), " Paciente"), /*#__PURE__*/React.createElement("i", {
      className: "fas fa-chevron-right text-gray-300 text-xs"
    }), /*#__PURE__*/React.createElement("div", {
      className: `flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium ${showAddressSelection ? 'bg-cyan-500 text-white' : 'bg-gray-100 text-gray-400'}`
    }, /*#__PURE__*/React.createElement("i", {
    ...
    }), /*#__PURE__*/React.createElement("p", null, "No se encontr\xF3 ning\xFAn ", entityName)) : (filteredPets || []).map(pet => /*#__PURE__*/React.createElement("div", {
      key: pet.id,
      onClick: () => assignPetToExam(pet),
      ...
      className: "text-2xl"
    }, selectedPetForOrder?.photo), /*#__PURE__*/React.createElement("div", {
      className: "flex-1"
    }, /*#__PURE__*/React.createElement("p", {
      className: "font-medium text-emerald-800"
    }, selectedPetForOrder?.name), /*#__PURE__*/React.createElement("p", {
      className: "text-xs text-emerald-600"
    }, selectedPetForOrder?.breed)), /*#__PURE__*/React.createElement("i", {
    ...
    }), /*#__PURE__*/React.createElement("p", null, "No tienes direcciones guardadas")) : (database.addresses || []).map(addr => /*#__PURE__*/React.createElement("div", {
      key: addr.id,
  ...
  if (freshUser.type === 'medico') {
    ...
    const filteredMascotas = mascotaSearchTerm.trim() === '' ? petsOrPatients : petsOrPatients.filter(p => (p.name || p.Name).toLowerCase().includes(mascotaSearchTerm.toLowerCase()) || p.owner && p.owner.toLowerCase().includes(mascotaSearchTerm.toLowerCase())); // Filtrar exámenes de una mascota según filtros
    ...
    const completedOrders = (database.orders || []).filter(o => (o.status === 9 || o.status === 'completed' || Number(o.status) === 9) && (o.items || []).length > 0).sort((a, b) => new Date(b.completedAt || b.createdAt) - new Date(a.completedAt || a.createdAt)); // Obtener pedidos en curso
    const pendingOrders = (database.orders || []).filter(o => o.status !== 9 && o.status !== 'completed' && Number(o.status) !== 9 && (o.items || []).length > 0).sort((a, b) => new Date(b.date || b.createdAt) - new Date(a.date || a.createdAt));
    (database.orders || []).forEach(o => {
      console.log("ORDER:", o);
      console.log("ADDRESS ID:", o?.addressId);
    }); // Obtener tipos de examen únicos para el filtro
    const allExamTypes = [...new Set((Array.isArray(petsOrPatients) ? petsOrPatients : []).flatMap(p => Array.isArray(p?.exams) ? p.exams : []).map(e => e?.type).filter(Boolean))];
    return /*#__PURE__*/React.createElement("div", {
      className: "min-h-screen bg-gray-100"
      ...
      onClick: () => cart.length > 0 && setShowOrderSummary(true),
      className: `relative w-9 h-9 rounded-xl flex items-center justify-center transition-colors ${cart.length > 0 ? 'bg-gray-100 text-gray-700' : 'bg-gray-100 text-gray-300 cursor-default'}`
    }, /*#__PURE__*/React.createElement("i", {
      className: "fas fa-shopping-cart text-base"
    }), cart.length > 0 && /*#__PURE__*/React.createElement("span", {
      className: "absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full text-white text-xs flex items-center justify-center font-bold"
      ...
      className: "fas fa-check-circle text-emerald-500"
    }), successMessage), viewingOrderTracking && (() => {
      ...
      return /*#__PURE__*/React.createElement("div", {
      ...
      }, getPetFullName(trackingOrder?.items?.[0]?.pet), " \xB7 ", trackingOrder?.items?.[0]?.examName || ''))), /*#__PURE__*/React.createElement("div", {
        className: "bg-white rounded-2xl shadow-sm border border-gray-100 p-4"
      }, /*#__PURE__*/React.createElement("h3", {
        className: "font-semibold text-xs uppercase tracking-wide text-gray-400 mb-3"
      }, "Detalle del pedido"), /*#__PURE__*/React.createElement("div", {
        className: "space-y-2"
      }, (trackingOrder?.items || []).map((step, index, arr) => /*#__PURE__*/React.createElement("div", {
        key: step.id || index,
        className: "flex items-center gap-3 p-2.5 bg-gray-50 rounded-xl"
      }, /*#__PURE__*/React.createElement("div", {
        className: `w-9 h-9 ${step?.exam?.bg} rounded-xl flex items-center justify-center flex-shrink-0`
      }, /*#__PURE__*/React.createElement("i", {
        className: `fas ${step?.exam?.icon} ${step?.exam?.color} text-sm`
      })), /*#__PURE__*/React.createElement("div", {
        className: "flex-1 min-w-0"
      }, /*#__PURE__*/React.createElement("p", {
        className: "font-medium text-gray-800 text-sm leading-tight truncate"
      }, step?.exam?.name), /*#__PURE__*/React.createElement("p", {
        className: "text-gray-500 text-xs"
      }, step?.pet?.photo, " ", getPetFullName(step.pet)))))), /*#__PURE__*/React.createElement("div", {
        className: "mt-3 pt-3 border-t border-gray-100"
      }, /*#__PURE__*/React.createElement("div", {
        className: "flex items-start gap-2 text-sm"
      }, /*#__PURE__*/React.createElement("i", {
      ...
      }, trackingOrder?.items?.[0]?.address?.address || 'Sin dirección', ", ", trackingOrder?.items?.[0]?.address?.district || ''))), trackingOrder?.comment && /*#__PURE__*/React.createElement("div", {
        ...
        return /*#__PURE__*/React.createElement("div", {
          ...
          className: `w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${isCompleted ? 'bg-cyan-500 text-white' : 'bg-gray-100 text-gray-400'}`
        }, isCompleted ? /*#__PURE__*/React.createElement("i", {
          className: "fas fa-check text-xs"
        }) : /*#__PURE__*/React.createElement("i", {
          className: `fas ${step.icon} text-xs`
        })), !isLast && /*#__PURE__*/React.createElement("div", {
          className: `w-0.5 h-9 mt-0.5 ${index < trackingCurrentIndex ? 'bg-cyan-400' : 'bg-gray-200'}`
        })), /*#__PURE__*/React.createElement("div", {
          className: `flex-1 min-w-0 ${!isLast ? 'pb-2' : ''}`
        }, /*#__PURE__*/React.createElement("p", {
          className: `font-medium text-sm leading-tight ${isCompleted ? 'text-gray-800' : 'text-gray-400'}`
        }, step.label), /*#__PURE__*/React.createElement("p", {
          className: `text-xs mt-0.5 ${isCompleted ? 'text-gray-500' : 'text-gray-400'}`
        }, step.subtitle), isCompleted && step.date && /*#__PURE__*/React.createElement("p", {
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: `flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${!showAddressSelection ? 'bg-cyan-500 text-white' : 'bg-emerald-100 text-emerald-700'}`
      }, /*#__PURE__*/React.createElement("i", {
        className: `fas ${!showAddressSelection ? 'fa-paw' : 'fa-check'} text-xs`
      }), " Paciente"), /*#__PURE__*/React.createElement("i", {
        className: "fas fa-chevron-right text-gray-300 text-xs"
      }), /*#__PURE__*/React.createElement("div", {
        className: `flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${showAddressSelection ? 'bg-cyan-500 text-white' : 'bg-gray-200 text-gray-400'}`
      }, /*#__PURE__*/React.createElement("i", {
      ...
      }, pendingExams.findIndex(e => e.id === currentExamForPet.id) + 1, "/", pendingExams.length)), !showAddressSelection && !showNewPetForm && /*#__PURE__*/React.createElement("div", {
        className: "px-4 pb-3"
      }, /*#__PURE__*/React.createElement("div", {
        className: "relative"
      }, /*#__PURE__*/React.createElement("i", {
      ...
      }, "No se encontr\xF3 ning\xFAn paciente") : (filteredPets || []).map(pet => /*#__PURE__*/React.createElement("div", {
        key: pet.id,
        onClick: () => assignPetToExam(pet),
      ...
      }, pet.breed, pet.owner ? ` · ${pet.owner}` : '')), /*#__PURE__*/React.createElement("i", {
        ...
        className: "bg-white rounded-xl p-6 text-center text-gray-400 text-sm shadow-sm border border-gray-100"
      }, "No hay direcciones guardadas") : (database.addresses || []).map(addr => /*#__PURE__*/React.createElement("div", {
        key: addr.id,
        onClick: () => assignAddressAndComplete(addr),
      ...
      className: `w-9 h-9 ${item?.exam?.bg || 'bg-cyan-100'} rounded-lg flex items-center justify-center flex-shrink-0`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas ${item?.exam?.icon} ${item?.exam?.color} text-sm`
    })), /*#__PURE__*/React.createElement("div", {
      className: "flex-1 min-w-0"
    }, /*#__PURE__*/React.createElement("p", {
      className: "font-semibold text-gray-800 text-sm leading-tight truncate"
    ...
    }, safeImage(item?.pet?.photo) && /*#__PURE__*/React.createElement("img", {
      src: item.petPhoto,
      ...
      onClick: () => removeFromCart(item.exam.id),
      ...
      className: `flex-1 py-2 rounded-lg text-sm font-medium ${invoiceType === 'boleta' ? 'bg-cyan-500 text-white' : 'bg-gray-100 text-gray-600'}`
    }, /*#__PURE__*/React.createElement("i", {
      className: "fas fa-receipt mr-1.5"
    }), "Boleta"), /*#__PURE__*/React.createElement("button", {
      onClick: () => setInvoiceType('factura'),
      className: `flex-1 py-2 rounded-lg text-sm font-medium ${invoiceType === 'factura' ? 'bg-amber-500 text-white' : 'bg-gray-100 text-gray-600'}`
    }, /*#__PURE__*/React.createElement("i", {
      ...
      rows: "2",
      maxLength: "500"
    }), cart.some(i => i.exam.price) && /*#__PURE__*/React.createElement("div", {
      className: "flex items-center justify-between py-2 border-t border-gray-100"
    }, /*#__PURE__*/React.createElement("span", {
      className: "text-sm text-gray-600 font-semibold"
    }, "Total estimado"), /*#__PURE__*/React.createElement("span", {
      className: "text-base font-bold text-cyan-600"
    }, "S/ ", cart.reduce((sum, i) => sum + (parseFloat(i.exam.price) || 0), 0).toFixed(2))), /*#__PURE__*/React.createElement("div", {
      ...
      onClick: () => submitOrder(selectedAddressId),
      className: `w-full py-2.5 rounded-xl font-semibold text-sm ${termsAccepted ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-md' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`
    }, /*#__PURE__*/React.createElement("i", {
      ...
      (database.orders || []).filter(o => o.invoiceRequested && o.invoiceStatus === 'pending').forEach(o => {
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        if (item.type === 'invoice') return /*#__PURE__*/React.createElement("div", {
          ...
          className: `fas ${order.documentType === 'boleta' ? 'fa-receipt' : 'fa-file-invoice-dollar'} text-amber-600`
        })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("p", {
          ...
          className: "text-xs text-gray-500"
        }, order?.items?.[0]?.examName || 'Pedido'))), /*#__PURE__*/React.createElement("span", {
        ...
        }, typeof firstItem?.pet?.photo === "string" ? firstItem.petPhoto : "🐾"), /*#__PURE__*/React.createElement("div", {
          className: "flex-1 min-w-0"
        }, /*#__PURE__*/React.createElement("p", {
          className: "font-semibold text-gray-800 text-sm leading-tight truncate"
        }, (order?.items || []).map(i => i?.pet ? getPetFullName(i.pet) : null).filter(Boolean).join(', ')), /*#__PURE__*/React.createElement("p", {
          className: "text-xs text-gray-500 truncate"
        }, typeof firstItem?.exam?.name === "string" ? firstItem.exam.name : typeof firstItem?.examName === "string" ? firstItem.examName : "Examen")), /*#__PURE__*/React.createElement("div", {
          className: `w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${order.status === 3 || order.status === 8 ? 'bg-red-500' : 'bg-cyan-500'}`
        }, /*#__PURE__*/React.createElement("i", {
      ...
      return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
        ...
        return items.slice(0, 2).map((item, idx) => {
          ...
          if (item.type === 'invoice') return /*#__PURE__*/React.createElement("div", {
            ...
            className: `fas ${order.documentType === 'boleta' ? 'fa-receipt' : 'fa-file-invoice-dollar'} text-amber-600 text-sm`
          })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("p", {
            className: "text-sm font-semibold text-gray-800 leading-tight"
          }, order.documentType === 'boleta' ? 'Boleta' : 'Factura', " solicitada"), /*#__PURE__*/React.createElement("p", {
            className: "text-xs text-gray-500"
          ...
          }, order?.items?.[0]?.pet?.photo || '🐾'), /*#__PURE__*/React.createElement("div", {
            className: "flex-1 min-w-0"
          }, /*#__PURE__*/React.createElement("p", {
            className: "font-semibold text-gray-800 text-sm leading-tight truncate"
          }, order?.items?.[0]?.examName || 'Pedido'), /*#__PURE__*/React.createElement("p", {
            className: "text-xs text-gray-500 truncate"
          }, order?.items?.[0]?.examName || '')), /*#__PURE__*/React.createElement("div", {
            className: `w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${order.status === 'moto_arrived' || order.status === 'results_uploaded' ? 'bg-red-500' : 'bg-cyan-500'}`
          }, /*#__PURE__*/React.createElement("i", {
            className: "fas fa-route text-white text-sm"
      ...
      return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
        onClick: () => {
          if (lastOrder?.resultPdfUrl) window.open(lastOrder.resultPdfUrl, '_blank');
        },
        ...
        className: "text-2xl leading-none"
      }, typeof firstItem?.pet?.photo === "string" ? firstItem.petPhoto : "🐾"), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("p", {
        className: "font-semibold text-gray-800 text-sm"
      }, getPetFullName(firstItem.pet)), /*#__PURE__*/React.createElement("p", {
        className: "text-xs text-gray-500"
      }, typeof firstItem?.exam?.name === "string" ? firstItem.exam.name : firstItem?.examName || "Examen"), /*#__PURE__*/React.createElement("p", {
        className: "text-xs text-gray-400"
      }, new Date(lastOrder.completedAt || lastOrder.createdAt).toLocaleDateString('es-ES', {
        day: 'numeric',
        month: 'short',
        year: 'numeric'
      })))), relatedExam?.pdfData && /*#__PURE__*/React.createElement("div", {
        className: `px-3 py-2 rounded-xl ${newResultHighlight ? 'bg-emerald-100 text-emerald-700' : 'bg-purple-100 text-purple-700'}`
      }, /*#__PURE__*/React.createElement("i", {
        className: "fas fa-file-pdf text-base"
      })))), /*#__PURE__*/React.createElement("div", {
        className: "flex justify-end"
      }, /*#__PURE__*/React.createElement("button", {
        onClick: () => setMedicoView('resultados'),
      ...
      return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
        onClick: () => setViewingInvoice(lastComprobante),
        ...
        className: `w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${newInvoiceHighlight ? 'bg-emerald-100' : lastComprobante.documentType === 'boleta' ? 'bg-cyan-100' : 'bg-amber-100'}`
      }, /*#__PURE__*/React.createElement("i", {
        className: `fas ${lastComprobante.documentType === 'boleta' ? 'fa-receipt' : 'fa-file-invoice-dollar'} ${newInvoiceHighlight ? 'text-emerald-600' : lastComprobante.documentType === 'boleta' ? 'text-cyan-600' : 'text-amber-600'}`
      })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("p", {
        className: "font-semibold text-gray-800 text-sm"
      }, lastComprobante.documentType === 'boleta' ? 'Boleta' : 'Factura'), /*#__PURE__*/React.createElement("p", {
        className: "text-xs text-gray-500"
      }, (lastComprobante?.items || []).map(i => getPetFullName(i?.pet)).join(', ')), /*#__PURE__*/React.createElement("p", {
        className: "text-xs text-gray-400"
        ...
        className: `px-3 py-2 rounded-xl ${newInvoiceHighlight ? 'bg-emerald-100 text-emerald-700' : lastComprobante.documentType === 'boleta' ? 'bg-cyan-100 text-cyan-700' : 'bg-amber-100 text-amber-700'}`
      }, /*#__PURE__*/React.createElement("i", {
        className: "fas fa-eye text-base"
      })))), /*#__PURE__*/React.createElement("div", {
        className: "flex justify-end"
      }, /*#__PURE__*/React.createElement("button", {
        onClick: () => setMedicoView('facturacion'),
        className: "text-cyan-500 text-base font-semibold pt-1 pb-0.5 mt-1 flex items-center gap-1"
      }, "Ver todos ", /*#__PURE__*/React.createElement("i", {
        className: "fas fa-chevron-right text-sm"
      }))));
      ...
      className: `fas ${clinPedidosSearchOpen ? 'fa-times' : 'fa-search'} text-white text-xs`
    })))), /*#__PURE__*/React.createElement("div", {
      className: `dueno-search-slide ${clinPedidosSearchOpen ? 'open' : ''}`
    }, /*#__PURE__*/React.createElement("div", {
      className: "relative"
    }, /*#__PURE__*/React.createElement("i", {
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: `w-11 h-11 ${exam?.bg} rounded-xl flex items-center justify-center flex-shrink-0`
      }, /*#__PURE__*/React.createElement("i", {
        className: `fas ${exam.icon} ${exam.color} text-lg`
      })), /*#__PURE__*/React.createElement("div", {
        className: "flex-1 min-w-0"
      }, /*#__PURE__*/React.createElement("p", {
    ...
    }))), (filteredCategorias || []).map(cat => /*#__PURE__*/React.createElement("div", {
      key: cat.nombre
    }, /*#__PURE__*/React.createElement("h3", {
      className: "text-sm font-bold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-2"
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas ${cat.icon} ${cat.color}`
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: `w-10 h-10 ${exam?.bg} rounded-xl flex items-center justify-center flex-shrink-0`
      }, /*#__PURE__*/React.createElement("i", {
        className: `fas ${exam.icon} ${exam.color} text-base`
      })), /*#__PURE__*/React.createElement("div", {
        className: "flex-1 min-w-0"
      }, /*#__PURE__*/React.createElement("p", {
        ...
        className: "text-gray-400 text-xs"
      }, exam.subtitle, exam.turnaround ? ` · ${exam.turnaround}` : "")), exam.price && /*#__PURE__*/React.createElement("span", {
        className: "text-cyan-600 font-bold text-sm flex-shrink-0"
      }, "S/ ", exam.price));
      ...
      className: `fas ${clinMascSearchOpen ? 'fa-times' : 'fa-search'} text-white text-xs`
    })))), /*#__PURE__*/React.createElement("div", {
      className: `dueno-search-slide ${clinMascSearchOpen ? 'open' : ''}`
    }, /*#__PURE__*/React.createElement("div", {
      className: "relative"
    }, /*#__PURE__*/React.createElement("i", {
      className: "fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm"
      ...
      className: "grid grid-cols-1 sm:grid-cols-2 gap-2"
    }, (filteredMascotas || []).map(pet => /*#__PURE__*/React.createElement("div", {
      key: pet.id,
      onClick: () => {
      ...
      className: "text-xs text-gray-400 truncate"
    }, pet.breed, pet.owner ? ` · ${pet.owner}` : '')), /*#__PURE__*/React.createElement("div", {
    ...
    }, (pet.exams || []).length)))))), !showOrderSummary && !selectedExam && !viewingInvoice && !viewingOrderTracking && medicoView === 'mascotas' && selectedMascota && /*#__PURE__*/React.createElement("div", {
      style: {
        position: 'relative',
      ...
      className: `fas ${clinMascDetailSearchOpen ? 'fa-times' : 'fa-search'} text-white text-xs`
    })))), /*#__PURE__*/React.createElement("div", {
      className: `dueno-search-slide-filters ${clinMascDetailSearchOpen ? 'open' : ''}`
    }, /*#__PURE__*/React.createElement("div", {
      ...
      value: examFilterDay,
      onChange: e => setExamFilterDay(e.target.value),
      ...
      value: ""
    }, "D\xEDa"), [...Array(31)].map((_, i) => /*#__PURE__*/React.createElement("option", {
      key: i + 1,
      ...
      value: ""
    }, "Mes"), ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'].map((m, i) => /*#__PURE__*/React.createElement("option", {
      key: i,
      value: String(i + 1).padStart(2, '0')
      ...
      value: ""
    }, "A\xF1o"), ['2026', '2025', '2024', '2023'].map(y => /*#__PURE__*/React.createElement("option", {
      key: y,
      value: y
      ...
      className: "space-y-3"
    }, filterExams(selectedMascota.exams).map(exam => /*#__PURE__*/React.createElement("div", {
      key: exam.id,
      onClick: () => {
      ...
      className: `w-10 h-10 rounded-xl flex items-center justify-center ${exam.seen === false ? 'bg-red-100' : 'bg-emerald-100'}`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas fa-file-pdf ${exam.seen === false ? 'text-red-600' : 'text-emerald-600'}`
    })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("p", {
      className: "font-semibold text-gray-800"
    }, exam.type), /*#__PURE__*/React.createElement("p", {
      className: "text-xs text-gray-500"
      ...
      className: `fas ${clinResSearchOpen ? 'fa-times' : 'fa-search'} text-white text-xs`
    })))), /*#__PURE__*/React.createElement("div", {
      className: `dueno-search-slide-filters ${clinResSearchOpen ? 'open' : ''}`
    }, /*#__PURE__*/React.createElement("div", {
      className: "relative mb-2"
    }, /*#__PURE__*/React.createElement("i", {
      className: "fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm"
      ...
      onChange: e => setResultadosSearch(e.target.value),
      ...
      onChange: e => setResultadosFilterDay(e.target.value),
      ...
      onChange: e => setResultadosFilterMonth(e.target.value),
      className: "flex-1 h-8 px-2 rounded-lg border border-gray-200 text-xs bg-gray-50 focus:border-purple-500 focus:outline-none appearance-none"
    }, /*#__PURE__*/React.createElement("option", {
      value: ""
    }, "Mes"), ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'].map((m, i) => /*#__PURE__*/React.createElement("option", {
      ...
      onChange: e => setResultadosFilterYear(e.target.value),
      className: "flex-1 h-8 px-2 rounded-lg border border-gray-200 text-xs bg-gray-50 focus:border-purple-500 focus:outline-none appearance-none"
    }, /*#__PURE__*/React.createElement("option", {
      value: ""
    }, "A\xF1o"), ['2026', '2025', '2024', '2023'].map(y => /*#__PURE__*/React.createElement("option", {
      ...
      return /*#__PURE__*/React.createElement("div", null, (filtered || []).length === 0 ? /*#__PURE__*/React.createElement("div", {
        ...
        return /*#__PURE__*/React.createElement("div", {
          ...
          className: `w-8 h-8 rounded-lg flex items-center justify-center ${relatedExam?.pdfData ? 'bg-purple-100 text-purple-600' : 'bg-gray-100 text-gray-400'}`
        }, /*#__PURE__*/React.createElement("i", {
          ...
          className: "font-medium text-gray-800"
        }, item?.pet?.name || ''), /*#__PURE__*/React.createElement("span", {
          className: "text-gray-300"
        }, "\u2022"), /*#__PURE__*/React.createElement("span", {
          className: "text-gray-600"
        }, item?.exam?.name || item?.examName || 'Examen')))));
      })));
      ...
      className: "bg-white/20 text-white text-xs px-2 py-0.5 rounded-full"
    }, completedOrders.filter(o => o.invoicePdf).length), /*#__PURE__*/React.createElement("button", {
      onClick: () => {
      ...
      className: `fas ${clinFacSearchOpen ? 'fa-times' : 'fa-search'} text-white text-xs`
    })))), /*#__PURE__*/React.createElement("div", {
      className: `dueno-search-slide-filters ${clinFacSearchOpen ? 'open' : ''}`
    }, /*#__PURE__*/React.createElement("div", {
      className: "relative mb-2"
    }, /*#__PURE__*/React.createElement("i", {
      className: "fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm"
      ...
      onChange: e => setFacturacionFilterDay(e.target.value),
      ...
      onChange: e => setFacturacionFilterMonth(e.target.value),
      className: "flex-1 h-8 px-2 rounded-lg border border-gray-200 text-xs bg-gray-50 focus:border-amber-500 focus:outline-none appearance-none"
    }, /*#__PURE__*/React.createElement("option", {
      value: ""
    }, "Mes"), ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'].map((m, i) => /*#__PURE__*/React.createElement("option", {
      ...
      onChange: e => setFacturacionFilterYear(e.target.value),
      className: "flex-1 h-8 px-2 rounded-lg border border-gray-200 text-xs bg-gray-50 focus:border-amber-500 focus:outline-none appearance-none"
    }, /*#__PURE__*/React.createElement("option", {
      value: ""
    }, "A\xF1o"), ['2026', '2025', '2024', '2023'].map(y => /*#__PURE__*/React.createElement("option", {
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: "space-y-3"
      }, pendingInvoices.map(order => /*#__PURE__*/React.createElement("div", {
        ...
        className: `fas ${order.documentType === 'boleta' ? 'fa-receipt' : 'fa-file-invoice'} text-red-600`
      })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("p", {
        className: "font-semibold text-gray-800"
      }, order.documentType === 'boleta' ? 'Boleta' : 'Factura', " Solicitada"), /*#__PURE__*/React.createElement("p", {
        className: "text-xs text-gray-500"
      ...
      const filteredFacturas = completedOrders.filter(order => {
        ...
        if (facturacionSearch) {
          const matchesPet = order.items.some(item => item.pet.name.toLowerCase().includes(facturacionSearch.toLowerCase()));
          if (!matchesPet) return false;
        } // Filtro por fecha
        const orderDate = new Date(order.createdAt || order.completedAt || order.date);
        const orderDay = String(orderDate.getDate()).padStart(2, '0');
      ...
      return /*#__PURE__*/React.createElement("div", null, (filteredFacturas || []).length === 0 ? /*#__PURE__*/React.createElement("div", {
        ...
        className: "space-y-3"
      }, (filteredFacturas || []).map((order, index) => /*#__PURE__*/React.createElement("div", {
        key: order.id,
        ...
        className: `w-10 h-10 rounded-xl flex items-center justify-center ${order.documentType === 'boleta' ? 'bg-cyan-100' : 'bg-amber-100'}`
      }, /*#__PURE__*/React.createElement("i", {
        className: `fas ${order.documentType === 'boleta' ? 'fa-receipt text-cyan-600' : 'fa-file-invoice text-amber-600'}`
      })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("p", {
        className: "font-semibold text-gray-800"
        ...
        className: `w-8 h-8 rounded-lg flex items-center justify-center ${order.invoicePdf ? 'bg-amber-100 text-amber-600' : 'bg-gray-100 text-gray-400'}`
      }, /*#__PURE__*/React.createElement("i", {
        className: "fas fa-eye text-sm"
      })), /*#__PURE__*/React.createElement("span", {
      ...
      className: `mob-nav-btn ${medicoView === 'inicio' && !currentExamForPet && !viewingOrderTracking ? 'active' : ''}`,
      onClick: () => {
        setMedicoView('inicio');
        setSelectedMascota(null);
        setMenuOpen(false);
      ...
      className: `mob-nav-btn ${medicoView === 'pedidos' && !currentExamForPet && !viewingOrderTracking ? 'active' : ''}`,
      onClick: () => {
      ...
      className: `mob-nav-btn ${medicoView === 'mascotas' && !currentExamForPet ? 'active' : ''}`,
      onClick: () => {
      ...
      className: `mob-nav-btn ${medicoView === 'resultados' && !currentExamForPet ? 'active' : ''}`,
      onClick: () => {
      ...
      className: `mob-nav-btn ${medicoView === 'facturacion' && !currentExamForPet ? 'active' : ''}`,
      onClick: () => {
      ...
      className: `w-8 h-8 rounded-lg flex items-center justify-center ${medicoView === 'inicio' && !selectedExam && !viewingInvoice && !currentExamForPet ? 'bg-white/20' : 'bg-cyan-100'}`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas fa-home ${medicoView === 'inicio' && !selectedExam && !viewingInvoice && !currentExamForPet ? 'text-white' : 'text-cyan-600'}`
    })), /*#__PURE__*/React.createElement("span", {
      className: "font-medium"
      ...
      className: `w-8 h-8 rounded-lg flex items-center justify-center ${medicoView === 'pedidos' && !selectedExam && !viewingInvoice && !currentExamForPet ? 'bg-white/20' : 'bg-blue-100'}`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas fa-clipboard-list ${medicoView === 'pedidos' && !selectedExam && !viewingInvoice && !currentExamForPet ? 'text-white' : 'text-blue-600'}`
    })), /*#__PURE__*/React.createElement("span", {
      className: "font-medium"
    }, "Pedidos")), /*#__PURE__*/React.createElement("button", {
      onClick: () => {
      ...
      className: `w-8 h-8 rounded-lg flex items-center justify-center ${medicoView === 'mascotas' && !selectedExam && !viewingInvoice && !currentExamForPet ? 'bg-white/20' : 'bg-emerald-100'}`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas fa-paw ${medicoView === 'mascotas' && !selectedExam && !viewingInvoice && !currentExamForPet ? 'text-white' : 'text-emerald-600'}`
    })), /*#__PURE__*/React.createElement("span", {
      className: "font-medium"
    }, "Pacientes"), /*#__PURE__*/React.createElement("span", {
      className: `ml-auto text-xs px-2 py-0.5 rounded-full ${medicoView === 'mascotas' && !selectedExam && !viewingInvoice && !currentExamForPet ? 'bg-white/20' : 'bg-emerald-100 text-emerald-600'}`
    }, (petsOrPatients || []).length)), /*#__PURE__*/React.createElement("button", {
      onClick: () => {
        setMedicoView('resultados');
        setShowAllPending(false);
      ...
      className: `w-8 h-8 rounded-lg flex items-center justify-center ${medicoView === 'resultados' && !selectedExam && !viewingInvoice && !currentExamForPet ? 'bg-white/20' : 'bg-purple-100'}`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas fa-file-medical ${medicoView === 'resultados' && !selectedExam && !viewingInvoice && !currentExamForPet ? 'text-white' : 'text-purple-600'}`
    })), /*#__PURE__*/React.createElement("span", {
      className: "font-medium"
    }, "Resultados"), (completedOrders || []).length > 0 && /*#__PURE__*/React.createElement("span", {
      className: `ml-auto text-xs px-2 py-0.5 rounded-full ${medicoView === 'resultados' && !selectedExam && !viewingInvoice && !currentExamForPet ? 'bg-white/20' : 'bg-purple-100 text-purple-600'}`
    }, completedOrders.length)), /*#__PURE__*/React.createElement("button", {
      onClick: () => {
        setMedicoView('facturacion');
        setShowAllPending(false);
      ...
      className: `w-8 h-8 rounded-lg flex items-center justify-center ${medicoView === 'facturacion' && !selectedExam && !viewingInvoice && !currentExamForPet ? 'bg-white/20' : 'bg-amber-100'}`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas fa-file-invoice-dollar ${medicoView === 'facturacion' && !selectedExam && !viewingInvoice && !currentExamForPet ? 'text-white' : 'text-amber-600'}`
    })), /*#__PURE__*/React.createElement("span", {
      className: "font-medium"
    }, "Facturaci\xF3n"))), /*#__PURE__*/React.createElement("div", {
      className: "mx-4 border-t border-gray-200"
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        return /*#__PURE__*/React.createElement("div", {
          ...
          className: `w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${isCompleted ? 'bg-cyan-500 text-white' : 'bg-gray-200 text-gray-400'}`
        }, isCompleted ? /*#__PURE__*/React.createElement("i", {
          className: "fas fa-check text-xs"
        }) : /*#__PURE__*/React.createElement("i", {
          className: `fas ${step.icon} text-xs`
        })), !isLast && /*#__PURE__*/React.createElement("div", {
          className: `w-0.5 h-10 ${index < trackingCurrentIndex ? 'bg-cyan-500' : 'bg-gray-200'}`
        })), /*#__PURE__*/React.createElement("div", {
          className: "flex-1"
        }, /*#__PURE__*/React.createElement("p", {
          className: `font-medium text-sm ${isCompleted ? 'text-gray-800' : 'text-gray-400'}`
        ...
        className: "space-y-2"
      }, (trackingOrder.items || []).map((item, idx) => /*#__PURE__*/React.createElement("div", {
        key: idx,
        className: "flex items-center gap-3 p-3 bg-gray-50 rounded-xl"
      }, /*#__PURE__*/React.createElement("div", {
        className: `w-10 h-10 ${item?.exam?.bg || 'bg-cyan-100'} rounded-lg flex items-center justify-center`
        ...
        className: "text-gray-500 text-xs"
      }, item?.pet?.photo, " ", item?.pet?.name || ''))))), /*#__PURE__*/React.createElement("div", {
        ...
        const addr = (database.addresses || []).find(a => String(a.id) === String(trackingOrder?.addressId));
        return addr ? `${addr.street}, ${addr.district}` : 'Sin dirección';
      })())))), trackingOrder.comment && /*#__PURE__*/React.createElement("div", {
        className: "mt-3 pt-3 border-t border-gray-100"
      }, /*#__PURE__*/React.createElement("div", {
        className: "flex items-start gap-2 text-sm"
      ...
      className: `fas ${viewingInvoice.documentType === 'boleta' ? 'fa-receipt text-cyan-500' : 'fa-file-invoice text-amber-500'} text-lg`
    }), /*#__PURE__*/React.createElement("span", {
      className: `${viewingInvoice.documentType === 'boleta' ? 'text-cyan-600' : 'text-amber-600'} font-semibold`
      ...
      className: `fas ${viewingInvoice.documentType === 'boleta' ? 'fa-receipt' : 'fa-file-invoice'} text-4xl text-gray-400 mb-2`
    }), /*#__PURE__*/React.createElement("p", {
      className: "text-gray-500"
      ...
      className: "text-2xl"
    }, selectedPetForOrder?.photo), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("p", {
      className: "font-medium text-emerald-800"
    }, selectedPetForOrder?.name), /*#__PURE__*/React.createElement("p", {
      className: "text-xs text-emerald-600"
    }, selectedPetForOrder?.breed))), /*#__PURE__*/React.createElement("div", {
      className: "bg-white rounded-2xl shadow overflow-hidden mb-4 relative"
    }, /*#__PURE__*/React.createElement("div", {
      ...
      return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
        ...
        return /*#__PURE__*/React.createElement("div", {
          ...
          className: `w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${order.status === 'moto_arrived' || order.status === 'results_uploaded' ? 'bg-red-500' : 'bg-cyan-500'}`
        }, /*#__PURE__*/React.createElement("i", {
          ...
          className: `fas ${invoice.documentType === 'boleta' ? 'fa-receipt' : 'fa-file-invoice-dollar'} text-amber-600`
        })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("p", {
          className: "font-semibold text-gray-800"
        }, invoice.documentType === 'boleta' ? 'Boleta' : 'Factura', " solicitada"), /*#__PURE__*/React.createElement("p", {
          className: "text-sm text-gray-500"
        }, invoice.items.map(i => getPetFullName(i.pet)).join(', ')))), /*#__PURE__*/React.createElement("span", {
      ...
      return /*#__PURE__*/React.createElement("div", {
        className: `bg-white rounded-xl p-4 shadow hover:shadow-lg transition-all cursor-pointer ${newResultHighlight ? 'highlight-green-pulse' : ''}`,
        onClick: () => exam?.pdfData && setSelectedExam(exam)
      }, /*#__PURE__*/React.createElement("div", {
        className: "flex items-center gap-4"
      }, /*#__PURE__*/React.createElement("span", {
        className: "text-3xl"
      }, firstItem?.pet?.photo || '🐾'), /*#__PURE__*/React.createElement("div", {
        className: "flex-1"
      }, /*#__PURE__*/React.createElement("p", {
        className: "font-medium text-gray-800"
      }, lastOrder?.items?.[0]?.pet ? getPetFullName(lastOrder.items[0].pet) : "Sin mascota"), /*#__PURE__*/React.createElement("p", {
        className: "text-sm text-gray-500"
      }, lastOrder?.items?.[0]?.examName || '' || "Sin examen"), /*#__PURE__*/React.createElement("p", {
        className: "text-xs text-gray-400"
      }, lastOrder?.completedAt ? new Date(lastOrder.completedAt).toLocaleDateString('es-ES', {
        day: 'numeric',
        month: 'long',
        year: 'numeric'
      }) : "")), /*#__PURE__*/React.createElement("div", {
        className: `w-10 h-10 rounded-xl flex items-center justify-center ${newResultHighlight ? 'bg-emerald-100' : 'bg-purple-100'}`
      }, /*#__PURE__*/React.createElement("i", {
        className: `fas fa-eye ${newResultHighlight ? 'text-emerald-600' : 'text-purple-600'}`
      }))));
    })()), /*#__PURE__*/React.createElement("div", {
      className: "mb-8"
    }, /*#__PURE__*/React.createElement("div", {
      ...
      return /*#__PURE__*/React.createElement("div", {
        className: `bg-white rounded-xl p-4 shadow hover:shadow-lg transition-all cursor-pointer ${newInvoiceHighlight ? 'highlight-green-pulse' : ''}`,
        onClick: () => setViewingInvoice(lastInvoice)
      }, /*#__PURE__*/React.createElement("div", {
        className: "flex items-center gap-4"
      }, /*#__PURE__*/React.createElement("div", {
        className: `w-12 h-12 rounded-xl flex items-center justify-center ${newInvoiceHighlight ? 'bg-emerald-100' : lastInvoice.documentType === 'boleta' ? 'bg-cyan-100' : 'bg-amber-100'}`
      }, /*#__PURE__*/React.createElement("i", {
        className: `fas ${lastInvoice.documentType === 'boleta' ? 'fa-receipt' : 'fa-file-invoice'} text-xl ${newInvoiceHighlight ? 'text-emerald-600' : lastInvoice.documentType === 'boleta' ? 'text-cyan-600' : 'text-amber-600'}`
      })), /*#__PURE__*/React.createElement("div", {
        className: "flex-1"
      }, /*#__PURE__*/React.createElement("p", {
        className: "font-medium text-gray-800"
        ...
        className: `w-10 h-10 rounded-xl flex items-center justify-center ${newInvoiceHighlight ? 'bg-emerald-100' : 'bg-amber-100'}`
      }, /*#__PURE__*/React.createElement("i", {
        className: `fas fa-eye ${newInvoiceHighlight ? 'text-emerald-600' : 'text-amber-600'}`
      }))));
    })())), !showOrderSummary && !selectedExam && !viewingInvoice && !currentExamForPet && !viewingOrderTracking && medicoView === 'pedidos' && /*#__PURE__*/React.createElement("div", {
      className: "flex flex-col h-full"
    }, /*#__PURE__*/React.createElement("div", {
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: `w-6 h-6 ${isInCart ? 'bg-emerald-500' : 'bg-amber-400'} rounded-full flex items-center justify-center flex-shrink-0`
      }, /*#__PURE__*/React.createElement("i", {
        className: `fas ${isInCart ? 'fa-check' : 'fa-clock'} text-white text-xs`
      })));
    }))), filteredCategorias.map(cat => /*#__PURE__*/React.createElement("div", {
      key: cat.nombre
    }, /*#__PURE__*/React.createElement("h3", {
    ...
    }, (cart || []).map(c => getPetFullName(c.pet)).join(', '))), /*#__PURE__*/React.createElement("button", {
    ...
    }, (pet.exams || []).length, " ex\xE1m."))))))), !selectedExam && !viewingInvoice && !currentExamForPet && !viewingOrderTracking && medicoView === 'mascotas' && selectedMascota && /*#__PURE__*/React.createElement("div", {
      className: "flex flex-col h-full"
    }, /*#__PURE__*/React.createElement("div", {
      ...
      key: i,
      value: String(i + 1).padStart(2, '0')
    }, m))), /*#__PURE__*/React.createElement("i", {
      className: "fas fa-chevron-down absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 text-xs pointer-events-none"
    })), /*#__PURE__*/React.createElement("div", {
      className: "relative"
    }, /*#__PURE__*/React.createElement("select", {
      ...
      key: y,
      value: y
    }, y))), /*#__PURE__*/React.createElement("i", {
      className: "fas fa-chevron-down absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 text-xs pointer-events-none"
    ...
    }, filterExams(selectedMascota.exams).sort((a, b) => new Date(b.date) - new Date(a.date)).map(exam => /*#__PURE__*/React.createElement("div", {
      key: exam.id,
      onClick: () => {
      ...
      className: `w-12 h-12 rounded-xl flex items-center justify-center ${exam.seen === false ? 'bg-red-100' : 'bg-purple-100'}`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas fa-file-pdf text-xl ${exam.seen === false ? 'text-red-600' : 'text-purple-600'}`
    })), /*#__PURE__*/React.createElement("div", {
      className: "flex-1"
    }, /*#__PURE__*/React.createElement("p", {
      className: "font-semibold text-gray-800"
      ...
      const completedOrders = (database.orders || []).filter(o => o.status === 9 || o.status === 'completed' || Number(o.status) === 9); // Filtrar resultados
      let filtered = completedOrders || [];
      if (resultadosSearch) {
        filtered = filtered.filter(order => order.items.some(item => (item.pet?.name || item.pet2?.name || '').toLowerCase().includes(resultadosSearch.toLowerCase()) || (item.exam?.name || '').toLowerCase().includes(resultadosSearch.toLowerCase())));
      }
      ...
      return /*#__PURE__*/React.createElement("div", {
        className: "grid grid-cols-2 gap-4 pb-6"
      }, filtered.map(order => /*#__PURE__*/React.createElement("div", {
        key: order.id,
        onClick: () => {
          if (order.resultPdfUrl) {
            const win = window.open();
            win.location.href = `https://inulab-backend-production.up.railway.app${order.resultPdfUrl}`;
          } else if (order.items?.[0]?.pdfData) {
            const win = window.open();
            win.location.href = order.items[0].pdfData;
        ...
        className: "font-medium text-gray-800"
      }, item?.pet?.name || ''), /*#__PURE__*/React.createElement("p", {
        className: "text-sm text-gray-500"
      }, item?.exam?.name || item?.examName || 'Examen')), /*#__PURE__*/React.createElement("div", {
    ...
    }, completedOrders.filter(o => o.invoicePdf).length)), /*#__PURE__*/React.createElement("div", {
      className: "relative flex-1"
    }, /*#__PURE__*/React.createElement("i", {
      className: "fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"
    }), /*#__PURE__*/React.createElement("input", {
      ...
      if (facturacionSearch) {
        invoices = invoices.filter(order => order.items.some(item => item.pet.name.toLowerCase().includes(facturacionSearch.toLowerCase())));
      }
      ...
      return /*#__PURE__*/React.createElement("div", {
        className: "grid grid-cols-2 gap-4 pb-6"
      }, invoices.map(order => /*#__PURE__*/React.createElement("div", {
        key: order.id,
        ...
        className: `w-12 h-12 rounded-xl flex items-center justify-center ${order.documentType === 'boleta' ? 'bg-cyan-100' : 'bg-amber-100'}`
      }, /*#__PURE__*/React.createElement("i", {
        className: `fas ${order.documentType === 'boleta' ? 'fa-receipt text-cyan-600' : 'fa-file-invoice text-amber-600'} text-xl`
      })), /*#__PURE__*/React.createElement("div", {
        className: "flex-1"
      }, /*#__PURE__*/React.createElement("p", {
        className: "font-medium text-gray-800"
        ...
        className: "text-sm text-gray-500"
      }, getPetFullName(order?.items?.[0]?.pet), " - ", order?.items?.[0]?.examName || ''), /*#__PURE__*/React.createElement("p", {
        className: "text-xs text-gray-400"
      ...
      className: `w-10 h-10 ${item?.exam?.bg || 'bg-cyan-100'} rounded-lg flex items-center justify-center flex-shrink-0`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas ${item?.exam?.icon || 'fa-vial'} ${item?.exam?.color || 'text-cyan-600'}`
    })), /*#__PURE__*/React.createElement("div", {
      className: "min-w-0"
      ...
      className: "font-medium text-gray-800 text-sm"
    }, item?.exam?.name || item?.examName || 'Examen'), item?.exam?.subtitle && /*#__PURE__*/React.createElement("p", {
      className: "text-xs text-gray-500 truncate"
    ...
    }), /*#__PURE__*/React.createElement("span", null, item.address?.name, ": ", item.address?.address, ", ", safeText(item?.address?.district)))))))), /*#__PURE__*/React.createElement("div", {
      ...
      onClick: () => setInvoiceType('boleta'),
      className: `py-2.5 px-3 rounded-xl text-sm font-medium transition-all ${invoiceType === 'boleta' ? 'bg-cyan-500 text-white shadow-md' : 'bg-white text-gray-600 border border-gray-200 hover:border-cyan-300'}`
    }, /*#__PURE__*/React.createElement("i", {
      className: "fas fa-receipt mr-1.5"
    }), "Boleta"), /*#__PURE__*/React.createElement("button", {
      onClick: () => setInvoiceType('factura'),
      className: `py-2.5 px-3 rounded-xl text-sm font-medium transition-all ${invoiceType === 'factura' ? 'bg-amber-500 text-white shadow-md' : 'bg-white text-gray-600 border border-gray-200 hover:border-amber-300'}`
    }, /*#__PURE__*/React.createElement("i", {
      className: "fas fa-file-invoice mr-1.5"
      ...
      className: `w-full py-3.5 rounded-xl font-semibold transition-all ${termsAccepted ? "bg-gradient-to-r from-cyan-500 to-blue-600 text-white" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`
    }, /*#__PURE__*/React.createElement("i", {
      className: "fas fa-paper-plane mr-2"
    }), "Solicitar Pedido")))))))));
  } // =====================================================
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    onClick: () => {
      ...
      a.download = `${selectedExam.type.replace(/[^a-zA-Z0-9]/g, '_')}_${selectedExam.date}.pdf`;
    ...
    className: "text-gray-500"
  }, "PDF no disponible")))))), viewingInvoice && !selectedExam && !viewingOrderTracking && (() => {
    const docType = viewingInvoice.documentType === 'boleta' ? 'Boleta' : 'Factura';
    return /*#__PURE__*/React.createElement("div", {
      className: "flex flex-col",
      style: {
      ...
      className: `font-semibold ${viewingInvoice.documentType === 'boleta' ? 'text-cyan-600' : 'text-amber-600'}`
      ...
      onClick: () => {
        ...
        a.download = `${docType}_${viewingInvoice.id}.pdf`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
      },
      className: `flex items-center gap-2 px-6 py-2.5 ${viewingInvoice.documentType === 'boleta' ? 'bg-gradient-to-r from-cyan-500 to-blue-600' : 'bg-gradient-to-r from-amber-500 to-orange-600'} text-white rounded-xl font-semibold text-sm shadow-lg`
    }, /*#__PURE__*/React.createElement("i", {
      className: "fas fa-download"
    ...
    return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("button", {
      ...
      className: `w-10 h-10 ${item?.exam?.bg || 'bg-cyan-100' || 'bg-gray-200'} rounded-lg flex items-center justify-center`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas ${item?.exam?.icon || 'fa-vial'} ${item?.exam?.color || 'text-cyan-600'}`
    })), /*#__PURE__*/React.createElement("div", {
      className: "flex-1"
      ...
      className: "text-gray-800"
    }, order?.items?.[0]?.address?.address || 'Sin dirección', ", ", order?.items?.[0]?.address?.district || '')))), order.comment && /*#__PURE__*/React.createElement("div", {
      className: "mt-3 pt-3 border-t border-gray-100"
    }, /*#__PURE__*/React.createElement("div", {
      className: "flex items-start gap-2 text-sm"
    }, /*#__PURE__*/React.createElement("i", {
    ...
    onClick: () => setFacturacionInvoice(null),
    className: "w-10 h-10 bg-white hover:bg-gray-100 rounded-full flex items-center justify-center text-gray-600 shadow-sm"
  }, /*#__PURE__*/React.createElement("i", {
    className: "fas fa-arrow-left"
  })), /*#__PURE__*/React.createElement("div", {
    ...
    className: `fas ${facturacionInvoice.documentType === 'boleta' ? 'fa-receipt text-cyan-500' : 'fa-file-invoice-dollar text-amber-500'} text-lg`
  }), /*#__PURE__*/React.createElement("span", {
    className: `font-semibold ${facturacionInvoice.documentType === 'boleta' ? 'text-cyan-600' : 'text-amber-600'}`
  ...
  }, facturacionInvoice?.invoicePdf ? /*#__PURE__*/React.createElement(PdfViewer, {
  ...
  }), /*#__PURE__*/React.createElement("p", null, "Comprobante no disponible")), facturacionInvoice?.invoicePdf && /*#__PURE__*/React.createElement("div", {
    ...
    className: "text-gray-500"
  }, "Comprobante no disponible")))))), !viewingOrderTracking && !selectedExam && !viewingInvoice && selectedPet && activeTab === 'resultados' && (() => {
    ...
    return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
      ...
      return true;
    }).sort((a, b) => new Date(b.date) - new Date(a.date)).map(exam => /*#__PURE__*/React.createElement("div", {
      key: exam.id,
      onClick: () => markExamAsSeen(exam, currentPet),
      ...
      className: `w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${exam.seen === false ? 'bg-red-100' : 'bg-purple-100'}`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas fa-file-pdf ${exam.seen === false ? 'text-red-600' : 'text-purple-600'}`
    })), /*#__PURE__*/React.createElement("div", {
      className: "flex-1 min-w-0"
    }, /*#__PURE__*/React.createElement("p", {
      className: "font-semibold text-gray-800 text-sm"
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "text-sm text-cyan-600 mt-1"
    }, showAddressSelection ? `${selectedPetForOrder?.name} - Selecciona dirección` : `Selecciona ${entityName}`))), /*#__PURE__*/React.createElement("div", {
      className: "flex items-center gap-2 mb-4"
    }, [{
      n: 1,
      l: 'Mascota',
      ...
      className: `flex-1 h-0.5 ${s.done || showAddressSelection ? 'bg-cyan-500' : 'bg-gray-200'}`
    }), /*#__PURE__*/React.createElement("div", {
      className: "flex items-center gap-2"
    }, /*#__PURE__*/React.createElement("div", {
      className: `w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${i === 0 && !showAddressSelection || i === 1 && showAddressSelection ? 'bg-cyan-500 text-white' : s.done ? 'bg-cyan-500 text-white' : 'bg-gray-200 text-gray-400'}`
    }, s.done ? '✓' : s.n), /*#__PURE__*/React.createElement("span", {
      className: `text-xs font-medium ${i === 0 && !showAddressSelection || i === 1 && showAddressSelection ? 'text-cyan-600' : 'text-gray-400'}`
    }, s.l))))), !showAddressSelection ? /*#__PURE__*/React.createElement(React.Fragment, null, !showNewPetForm ? /*#__PURE__*/React.createElement("div", {
      className: "space-y-3"
    }, petsOrPatients.map(pet => /*#__PURE__*/React.createElement("div", {
      key: pet.id,
      onClick: () => assignPetToExam(pet),
      ...
      onClick: () => completeTomaMuestraStep(false),
      ...
      className: "text-xs text-gray-400"
    }, "Solo necesito que la recojan"))), (() => {
      const tmPrice = getTomaMuestraPrice(pendingOrderAddress.district, examTotal + (currentExamForPet?.price || 0));
      return /*#__PURE__*/React.createElement("button", {
        onClick: () => completeTomaMuestraStep(true),
        className: "py-3.5 px-4 rounded-xl border-2 border-transparent bg-white shadow hover:shadow-lg hover:border-cyan-400 text-gray-700 font-medium transition-all flex items-center gap-3 cursor-pointer"
      }, /*#__PURE__*/React.createElement("div", {
        className: "w-10 h-10 bg-cyan-100 rounded-xl flex items-center justify-center flex-shrink-0"
      }, /*#__PURE__*/React.createElement("i", {
        ...
        className: "text-xs text-gray-400"
      }, tmPrice !== null ? `+S/ ${tmPrice} por el servicio a domicilio` : 'Consultar precio para tu distrito')));
    })()), /*#__PURE__*/React.createElement("button", {
    ...
    }, "No tienes direcciones guardadas") : (database.addresses || []).map(addr => /*#__PURE__*/React.createElement("div", {
      key: addr.id,
      onClick: () => handleDuenoAddressSelect(addr),
    ...
    className: `fas ${pedidosSearchOpen ? 'fa-times' : 'fa-search'} text-white text-xs`
  })))), /*#__PURE__*/React.createElement("div", {
    className: `dueno-search-slide ${pedidosSearchOpen ? 'open' : ''}`
  }, /*#__PURE__*/React.createElement("div", {
    className: "relative"
  }, /*#__PURE__*/React.createElement("i", {
    className: "fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm"
  ...
  }), pedidosSearchOpen && /*#__PURE__*/React.createElement("div", {
    className: "lg:hidden dueno-search-overlay",
    onClick: () => {
      setPedidosSearchOpen(false);
      setPedidosSearchTerm('');
    },
    style: {
      position: 'absolute',
      top: 0,
    ...
    return /*#__PURE__*/React.createElement("div", {
      key: paquete.id,
      onClick: () => handleExamClick(paquete),
      ...
      className: `w-14 h-14 ${paquete.bg} rounded-2xl flex items-center justify-center flex-shrink-0`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas ${paquete.icon} ${paquete.color} text-2xl`
    })), /*#__PURE__*/React.createElement("div", {
      className: "flex-1 min-w-0"
    }, /*#__PURE__*/React.createElement("h3", {
      ...
      onClick: e => {
        ...
        document.querySelector('.app-content-dueno')?.scrollTo(0, 0);
      },
      ...
      className: `w-7 h-7 ${isInCart ? 'bg-emerald-500' : 'bg-amber-400'} rounded-full flex items-center justify-center`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas ${isInCart ? 'fa-check' : 'fa-clock'} text-white text-sm`
    })))));
  })), /*#__PURE__*/React.createElement("div", {
    className: "mt-8"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: () => setShowTechnicalExams(!showTechnicalExams),
    ...
    className: `fas fa-chevron-${showTechnicalExams ? 'up' : 'down'} text-gray-400 transition-transform`
  ...
  }))), categoriasDuenos.map(cat => /*#__PURE__*/React.createElement("div", {
    key: cat.nombre
  }, /*#__PURE__*/React.createElement("h3", {
    className: "text-sm font-bold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-2"
  }, /*#__PURE__*/React.createElement("i", {
    ...
    className: `fas ${selectedPaqueteDetail.icon} ${selectedPaqueteDetail.color} text-3xl`
    ...
    className: "space-y-1"
  }, selectedPaqueteDetail.incluye.map((item, idx) => /*#__PURE__*/React.createElement("li", {
    key: idx,
    ...
    className: "space-y-3"
  }, selectedPaqueteDetail.evalua.map((item, idx) => /*#__PURE__*/React.createElement("div", {
    key: idx,
    ...
    className: "fas fa-map-marker-alt mt-0.5 text-cyan-500"
  }), /*#__PURE__*/React.createElement("span", null, item.address?.name, ": ", item.address?.address, ", ", safeText(item?.address?.district))), item.tomaMuestra && /*#__PURE__*/React.createElement("div", {
    ...
    const p = getTomaMuestraPrice(item.address?.district, examTotal);
    return p !== null ? `(+S/ ${p})` : '';
  })()))))))), /*#__PURE__*/React.createElement("div", {
    className: "lg:w-1/2 p-6"
  }, /*#__PURE__*/React.createElement("p", {
    ...
    className: "bg-purple-500 text-white text-xs px-2 py-0.5 rounded-full"
  }, (database.pets || []).reduce((sum, p) => sum + (p.exams?.length || 0), 0))), /*#__PURE__*/React.createElement("div", {
    className: "relative flex-1"
  }, /*#__PURE__*/React.createElement("i", {
    className: "fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"
  }), /*#__PURE__*/React.createElement("input", {
    ...
    value: resFilterDay,
    onChange: e => setResFilterDay(e.target.value),
    className: "h-10 w-24 pl-4 pr-8 rounded-full bg-white border border-gray-300 text-sm focus:border-purple-500 appearance-none cursor-pointer focus:outline-none"
  }, /*#__PURE__*/React.createElement("option", {
    value: ""
  }, "D\xEDa"), [...Array(31)].map((_, i) => /*#__PURE__*/React.createElement("option", {
    ...
    onChange: e => setResFilterMonth(e.target.value),
    className: "h-10 w-28 pl-4 pr-8 rounded-full bg-white border border-gray-300 text-sm focus:border-purple-500 appearance-none cursor-pointer focus:outline-none"
  }, /*#__PURE__*/React.createElement("option", {
    value: ""
  }, "Mes"), ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'].map((m, i) => /*#__PURE__*/React.createElement("option", {
    ...
    value: resFilterYear,
    onChange: e => setResFilterYear(e.target.value),
    className: "h-10 w-28 pl-4 pr-8 rounded-full bg-white border border-gray-300 text-sm focus:border-purple-500 appearance-none cursor-pointer focus:outline-none"
  }, /*#__PURE__*/React.createElement("option", {
    value: ""
  }, "A\xF1o"), ['2026', '2025', '2024', '2023'].map(y => /*#__PURE__*/React.createElement("option", {
    ...
    const userOrders = (database.orders || []).filter(o => o.status !== 'completed' && o.status !== 9 && Number(o.status) !== 9).filter(o => !resSearchTerm || (o.items || []).some(i => (i.examName || '').toLowerCase().includes(resSearchTerm.toLowerCase()) || (i.pet?.name || '').toLowerCase().includes(resSearchTerm.toLowerCase())));
    if ((userOrders || []).length > 0) {
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: "grid grid-cols-1 lg:grid-cols-2 gap-4"
      }, (userOrders || []).map(order => /*#__PURE__*/React.createElement("div", {
        key: order.id,
      ...
      }, item?.examName || item?.exam?.name || 'Examen'), /*#__PURE__*/React.createElement("p", {
        className: "text-gray-500 text-xs"
      }, item?.pet?.name || item?.petName || ''))))), order.status === 'moto_arrived' && /*#__PURE__*/React.createElement("div", {
    ...
    className: "text-lg font-bold text-gray-800 mb-4"
  }, "Selecciona tu mascota"), (database.pets || []).filter(p => !resSearchTerm || (p.name || '').toLowerCase().includes(resSearchTerm.toLowerCase())).length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "bg-white rounded-2xl p-10 text-center shadow"
  }, /*#__PURE__*/React.createElement("i", {
    className: "fas fa-paw text-4xl text-gray-300 mb-2"
  }), /*#__PURE__*/React.createElement("p", {
  ...
  }, (database.pets || []).filter(p => !resSearchTerm || (p.name || '').toLowerCase().includes(resSearchTerm.toLowerCase())).map(pet => /*#__PURE__*/React.createElement("div", {
    key: pet.id,
  ...
  }, (pet.exams || []).length)))))), medicoView === 'facturacion' && (() => {
    ...
    if (facInvoice) {
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: `fas ${facInvoice.documentType === 'boleta' ? 'fa-receipt text-cyan-500' : 'fa-file-invoice-dollar text-amber-500'} text-lg`
      }), /*#__PURE__*/React.createElement("span", {
        className: `font-semibold ${facInvoice.documentType === 'boleta' ? 'text-cyan-600' : 'text-amber-600'}`
        ...
        onClick: () => {
          ...
          a.download = `comprobante-${facInvoice.id}.pdf`;
          document.body.appendChild(a);
          a.click();
          document.body.removeChild(a);
        },
        className: `flex items-center gap-2 px-6 py-2.5 ${facInvoice.documentType === 'boleta' ? 'bg-gradient-to-r from-cyan-500 to-blue-600' : 'bg-gradient-to-r from-amber-500 to-orange-600'} text-white rounded-xl font-semibold transition-colors shadow-lg text-sm`
      }, /*#__PURE__*/React.createElement("i", {
        className: "fas fa-download"
      }), " Descargar PDF"))) : /*#__PURE__*/React.createElement("div", {
        className: "flex-1 bg-gray-100 rounded-xl flex flex-col items-center justify-center"
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "grid grid-cols-1 lg:grid-cols-2 gap-4"
    }, (userInvoices || []).map(inv => /*#__PURE__*/React.createElement("div", {
      key: inv.id,
      onClick: () => setFacInvoice(inv),
      ...
      className: `w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${inv.documentType === 'boleta' ? 'bg-cyan-100' : 'bg-amber-100'}`
    }, /*#__PURE__*/React.createElement("i", {
      className: `fas ${inv.documentType === 'boleta' ? 'fa-receipt text-cyan-600' : 'fa-file-invoice-dollar text-amber-600'} text-xl`
    })), /*#__PURE__*/React.createElement("div", {
      className: "flex-1 min-w-0"
    }, /*#__PURE__*/React.createElement("p", {
      className: "font-semibold text-gray-800"
      ...
      className: "text-sm text-gray-500"
    }, inv.items?.map(it => it.pet?.name).filter(Boolean).join(', ')), /*#__PURE__*/React.createElement("p", {
      ...
      className: `text-xs px-2 py-1 rounded-full font-medium ${inv.documentType === 'boleta' ? 'bg-cyan-100 text-cyan-700' : 'bg-amber-100 text-amber-700'}`
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: `fas ${facDuenoSearchOpen ? 'fa-times' : 'fa-search'} text-white text-xs`
    })))), /*#__PURE__*/React.createElement("div", {
      className: `dueno-search-slide-filters ${facDuenoSearchOpen ? 'open' : ''}`
    }, /*#__PURE__*/React.createElement("div", {
      className: "relative mb-2"
    }, /*#__PURE__*/React.createElement("i", {
      className: "fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm"
      ...
      onChange: e => setFacDuenoSearchTerm(e.target.value),
      placeholder: "Buscar comprobante...",
      className: "w-full h-9 pl-9 pr-8 border border-gray-200 focus:border-amber-500 focus:outline-none bg-gray-50 text-xs dueno-search-pill"
    }), facDuenoSearchTerm && /*#__PURE__*/React.createElement("button", {
      onClick: () => setFacDuenoSearchTerm(''),
      className: "absolute right-3 top-1/2 -translate-y-1/2 text-gray-400"
    }, /*#__PURE__*/React.createElement("i", {
      className: "fas fa-times text-sm"
    }))), /*#__PURE__*/React.createElement("div", {
      ...
      onChange: e => setFacDuenoFilterDay(e.target.value),
      className: "flex-1 h-8 px-2 rounded-lg border border-gray-200 text-xs bg-gray-50 focus:border-amber-500 focus:outline-none appearance-none"
    }, /*#__PURE__*/React.createElement("option", {
      value: ""
    }, "D\xEDa"), [...Array(31)].map((_, i) => /*#__PURE__*/React.createElement("option", {
      ...
      onChange: e => setFacDuenoFilterMonth(e.target.value),
      className: "flex-1 h-8 px-2 rounded-lg border border-gray-200 text-xs bg-gray-50 focus:border-amber-500 focus:outline-none appearance-none"
    }, /*#__PURE__*/React.createElement("option", {
      value: ""
    }, "Mes"), ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'].map((m, i) => /*#__PURE__*/React.createElement("option", {
      ...
      onChange: e => setFacDuenoFilterYear(e.target.value),
      className: "flex-1 h-8 px-2 rounded-lg border border-gray-200 text-xs bg-gray-50 focus:border-amber-500 focus:outline-none appearance-none"
    }, /*#__PURE__*/React.createElement("option", {
      value: ""
    }, "A\xF1o"), ['2026', '2025', '2024', '2023'].map(y => /*#__PURE__*/React.createElement("option", {
      ...
      const d = new Date(inv.fecha || inv.createdAt || inv.date);
      ...
      return true;
    }).map(inv => /*#__PURE__*/React.createElement("div", {
      key: inv.id,
      onClick: () => setFacturacionInvoice(inv),
      className: "bg-white rounded-2xl p-5 shadow-sm hover:shadow-md cursor-pointer transition-all border border-transparent hover:border-amber-200"
    }, /*#__PURE__*/React.createElement("div", {
      className: "flex items-center gap-4"
    }, /*#__PURE__*/React.createElement("div", {
    ...
    className: `mob-nav-btn ${activeTab === 'pedidos' && duenoView !== 'config' ? 'active' : ''}`,
    onClick: () => {
      resetDuenoState();
      setActiveTab('pedidos');
    }
    ...
    className: `mob-nav-btn ${activeTab === 'facturacion' && duenoView !== 'config' ? 'active' : ''}`,
    onClick: () => {
      resetDuenoState();
      setActiveTab('facturacion');
    }
    ...
    className: `mob-nav-btn ${activeTab === 'resultados' && duenoView !== 'config' ? 'active' : ''}`,
    onClick: () => {
      resetDuenoState();
      setActiveTab('resultados');
    }
