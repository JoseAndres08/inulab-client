            ...
            if (selectedExam && freshUser.type !== 'medico' && window.innerWidth < 768) {
                const downloadPdf = () => {
                    if (selectedExam.pdfData) {
                       fetch(selectedExam.pdfData, { headers: { 'Authorization': `Bearer ${localStorage.getItem('inulab_token')}` } }).then(r => r.blob()).then(blob => { const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'resultado.pdf'; document.body.appendChild(a); a.click(); document.body.removeChild(a); });am.pdfData; a.download = 'resultado.pdf'; document.body.appendChild(a); a.click(); document.body.removeChild(a);
                    }
                };
