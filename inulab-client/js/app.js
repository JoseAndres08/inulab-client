                        ...
                        <div style={{flex:1, overflow:'hidden', display:'flex', flexDirection:'column', padding:'12px', gap:'10px'}}>
                            ...
                            {selectedExam.pdfData && (
                                <div style={{flexShrink:0, display:'flex', justifyContent:'center'}}>
                                    <a href={selectedExam.pdfData} target="_blank" rel="noopener" className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-white rounded-xl font-semibold text-sm shadow-lg">
                ...
                return (
                    <div className="min-h-screen bg-gray-100">
                        ...
                        <div className="md:hidden">
                            ...
                            {!selectedExam && !viewingInvoice && !viewingOrderTracking && medicoView === 'configuracion' && showEditProfile && (
                                <div className="max-w-md mx-auto" style={{ overflowY: 'auto', height: '100vh' }}>
                                    <div className="bg-white rounded-2xl shadow-lg p-6">
                        ...
                        <div className="hidden md:block min-h-screen">
                            ...
                            <div className="pt-16 flex">
                                ...
                                <div className="ml-56 flex-1 p-6 bg-gray-100 h-[calc(100vh-64px)] overflow-hidden">
                                                    ...
                                                    <div className="bg-white rounded-2xl shadow-lg p-5 h-full flex flex-col">
                                                        ...
                                                        {selectedExam.pdfData ? (
                                                            <div className="flex-1 flex flex-col min-h-0">
                                                                <PdfViewer url={selectedExam.pdfData} originalUrl={selectedExam.pdfData} className="flex-1 w-full rounded-xl border border-gray-200 min-h-0" />
                                    ...
                                    {!selectedExam && !viewingInvoice && !currentExamForPet && !viewingOrderTracking && medicoView === 'configuracion' && showEditProfile && (
                                        <div className="max-w-lg mx-auto pb-20 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 80px)' }}>
                                            <div className="bg-white rounded-2xl shadow p-6">
                                                <div className="flex items-center gap-3 mb-6">
                                                    <button onClick={() => setShowEditProfile(false)} className="w-10 h-10 bg-gray-100 hover:bg-gray-200 rounded-xl flex items-center justify-center">
                                                        <i className="fas fa-arrow-left text-gray-600"></i>
                                ...
                                <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-4" onClick={() => setSelectedPaqueteDetail(null)}>
                                    <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full max-w-lg max-h-[85vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
                                        ...
                                        {activeTab === 'resultados' && (<div>
                                            {/* Header fijo con titulo + buscador + filtros integrados */}
                                            <div className="flex-shrink-0 bg-gray-100 shadow-md border-b border-gray-300 p-4 -mx-4 -mt-4 mb-4 lg:-mx-6 lg:-mt-6 lg:mb-6">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div className="relative flex-1">
                                                        <select value={resFilterDay} onChange={(e) => setResFilterDay(e.target.value)} className="w-full h-9 pl-3 pr-7 rounded-full bg-white border border-gray-300 text-sm focus:border-purple-500 appearance-none cursor-pointer focus:outline-none">
                                                            <option value="">Día</option>{[...Array(31)].map((_, i) => <option key={i + 1} value={String(i + 1).padStart(2, '0')}>{i + 1}</option>)}
                                                        </select>
                                                        <i className="fas fa-chevron-down absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 text-xs pointer-events-none"></i>
                                                    </div>
                                                    <div className="relative flex-1">
                                                        <select value={resFilterMonth} onChange={(e) => setResFilterMonth(e.target.value)} className="w-full h-9 pl-3 pr-7 rounded-full bg-white border border-gray-300 text-sm focus:border-purple-500 appearance-none cursor-pointer focus:outline-none">
                                                    ...
                                                    <div className="relative flex-1">
                                                        <select value={resFilterYear} onChange={(e) => setResFilterYear(e.target.value)} className="w-full h-9 pl-3 pr-7 rounded-full bg-white border border-gray-300 text-sm focus:border-purple-500 appearance-none cursor-pointer focus:outline-none">
                                            ...
                                            {(() => {
                                                ...
                                                ).filter(o => !resSearchTerm || (o.items || []).some(i => (i.examName || '').toLowerCase().includes(resSearchTerm.toLowerCase()) || (i.pet?.name || '').toLowerCase().includes(resSearchTerm.toLowerCase())));
