        ...
        // =============================================================================
        const API_BASE = "https://inulab-backend-production.up.railway.app";
        ...
        const api = {
            ...
            async updateProfile(profileData) {
                ...
                if (!res.ok) { const err = await res.json(); throw new Error(err.error || 'Error al actualizar perfil'); }
                return await res.json(); // { user }
            },
            ...
            async getPets() {
                const res = await fetch(`${API_BASE}/pets`, { headers: this._headers() });
            ...
            async getAddresses() {
                const res = await fetch(`${API_BASE}/addresses`, { headers: this._headers() });
            ...
            async getOrders() {
                const res = await fetch(`${API_BASE}/orders`, { headers: this._headers() });
            ...
            // --- CONFIG ---
            async getConfig() {
                const res = await fetch(`${API_BASE}/config`, { headers: this._headers() });
                if (!res.ok) return { nightMode: false };
                return await res.json(); // { nightMode }
            }
        };
        ...
        const App = () => {
            ...
            const handleLogout = () => {
                ...
                setIsAuthenticated(false); setUserType(null); setCurrentUser(null); setDatabase(null);
                setSelectedPet(null); setSelectedExam(null);
            ...
            const saveProfile = async (changingName = false) => {
                ...
                try {
                    ...
                    setDatabase(prev => ({ ...prev, users: prev.users.map((u, i) => i === userIndex ? user : u) }));
                    setCurrentUser(user);
            ...
            const addNewAddress = async () => {
                ...
                let addrToSave = { id: 'ADDR' + Date.now(), ...newAddress };
                // TODO: const { address } = await api.createAddress(newAddress); addrToSave = address;
                setDatabase(prev => {
                    const userIndex = prev.users.findIndex(u => u.id === currentUser.id);
                    const user = prev.users[userIndex];
                    const updatedUser = { ...user, addresses: [...(user.addresses || []), addrToSave] };
                    return { ...prev, users: prev.users.map((u, i) => i === userIndex ? updatedUser : u) };
                });
                if (freshUser?.type === 'dueño') {
            ...
            const addNewPet = async () => {
                const freshUser = database.users.find(u => u.id === currentUser.id);
                if (!newPet.name || !newPet.animalType || !newPet.breed || !newPet.sex) { alert('Por favor completa todos los campos'); return; }
                if (!newPet.ageValue || parseInt(newPet.ageValue) < 0) { alert('Por favor ingresa la edad'); return; }
                if (freshUser.type === 'medico' && !newPet.owner) { alert('Por favor ingresa el nombre del dueño'); return; }
                const petIcon = newPet.animalType === 'perro' ? '🐶' : newPet.animalType === 'gato' ? '🐱' : newPet.animalType === 'ave' ? '🦜' : newPet.animalType === 'conejo' ? '🐰' : '🐾';
                let petToSave = { id: 'PET' + Date.now(), name: newPet.name, owner: freshUser.type === 'medico' ? newPet.owner : `${freshUser.name} ${freshUser.lastName}`,
                    animalType: newPet.animalType, breed: newPet.breed, ageValue: parseInt(newPet.ageValue) || 0, ageUnit: newPet.ageUnit, sex: newPet.sex, photo: petIcon, exams: [] };
                // TODO: const { pet } = await api.createPet(petToSave); petToSave = pet;
                setDatabase(prev => {
                    const userIndex = prev.users.findIndex(u => u.id === currentUser.id);
                    const user = prev.users[userIndex];
                    const petsArr = user.type === 'medico' ? 'patients' : 'pets';
                    const updatedUser = { ...user, [petsArr]: [...(user[petsArr] || []), petToSave] };
                    return { ...prev, users: prev.users.map((u, i) => i === userIndex ? updatedUser : u) };
                });
            ...
            const submitOrder = async () => {
                const freshUser = database.users.find(u => u.id === currentUser.id) || currentUser;
                const newOrder = {
                    id: 'ORD' + Date.now(), userId: currentUser.id, userName: `${freshUser.name} ${freshUser.lastName || ''}`.trim(),
                    userType: freshUser.type, date: new Date().toISOString(), status: 'pending',
                    documentType: invoiceType, comment: orderComment.trim() || null,
                    invoiceRequested: true, invoiceStatus: 'pending',
                    items: cart.map(item => ({
                        exam: { id: item.exam.id, name: item.exam.name, subtitle: item.exam.subtitle, icon: item.exam.icon, color: item.exam.color, bg: item.exam.bg, price: item.exam.price || null, turnaround: item.exam.turnaround || null },
                        pet: { id: item.pet.id, name: item.pet.name, photo: item.pet.photo, breed: item.pet.breed, owner: item.pet.owner },
                ...
                setCart([]); setPendingExams([]); setCurrentExamForPet(null); setShowOrderSummary(false); setInvoiceType('factura'); setOrderComment(''); setTermsAccepted(false); setMedicoView('inicio');
            };
                ...
                if (exam.seen === false) {
                    try {
                        ...
                        setDatabase(prev => {
                            ...
                            return { ...prev, users: prev.users.map((u, i) => i === userIndex ? { ...u, [petsArray]: updatedPets } : u) };
                        });
                        ...
                        <div className="w-full max-w-md">
                            <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
                                <div className="flex flex-col items-center justify-center mb-6">
                                    <img src="assets/logo_inulaboratorios.jpg" alt="INULABORATORIOS" className="h-14 mb-2" onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex'; }} />
                                ...
                                <form onSubmit={handleLogin} className="space-y-3">
                                    <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full px-3.5 py-3 rounded-xl bg-gray-50 border border-gray-200 text-gray-800 text-sm placeholder-gray-400 focus:border-cyan-500 focus:bg-white focus:outline-none" placeholder="Usuario" />
                                    <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-3.5 py-3 rounded-xl bg-gray-50 border border-gray-200 text-gray-800 text-sm placeholder-gray-400 focus:border-cyan-500 focus:bg-white focus:outline-none" placeholder="Contraseña" />
                                    {error && <div className="bg-red-50 border border-red-200 p-2.5 rounded-xl"><p className="text-xs text-red-600">{error}</p></div>}
                                    <button type="submit" className="w-full py-3 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-cyan-400 to-cyan-500 hover:from-cyan-500 hover:to-cyan-600 transition-all shadow-md">Iniciar Sesión</button>
                                </form>
                            </div>
                        </div>
                    </div>
            ...
            if (selectedExam && freshUser.type !== 'medico') {
                ...
                return (
                    <div style={{position:'fixed', top:0, left:0, right:0, bottom:0, display:'flex', flexDirection:'column', background:'white'}}>
                        {/* Header */}
                        <div className="app-header border-b border-gray-100">
                            <div className="max-w-4xl mx-auto px-4 py-3 flex items-center gap-3">
                                <button onClick={() => setSelectedExam(null)} className="w-9 h-9 bg-gray-100 rounded-full flex items-center justify-center text-gray-600">
                                    <i className="fas fa-arrow-left text-sm"></i>
                                </button>
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-bold text-gray-800 leading-tight truncate">{selectedExam.type}</p>
                                    <p className="text-gray-400 text-xs"><i className="fas fa-calendar mr-1"></i>{new Date(selectedExam.date).toLocaleDateString('es-ES', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                                </div>
                                {selectedExam.pdfData && (
                                    <button onClick={downloadPdf} className="flex items-center gap-1.5 px-3 py-2 bg-cyan-500 text-white rounded-lg text-xs font-semibold">
                                        <i className="fas fa-download text-xs"></i>PDF
                                    </button>
                                )}
                            </div>
                        </div>
                        {/* PDF fills remaining space */}
                        <div style={{flex:1, paddingTop:'calc(56px + env(safe-area-inset-top, 0px))', overflow:'hidden'}}>
                            {selectedExam.pdfData ? (
                                <iframe src={selectedExam.pdfData} className="w-full h-full" title="PDF" />
                            ) : (
                    ...
                    <div className="min-h-screen bg-gray-100">
                        <div className="app-header">
                            <div className="max-w-4xl mx-auto px-4 py-3 flex items-center gap-4">
                                <button onClick={() => setViewingInvoice(null)} className="text-gray-600 hover:text-gray-800">
                        ...
                        <div className="app-content">
                            ...
                            <div className="bg-white rounded-2xl shadow-lg p-4">
                                <div className="mb-3">
                                    <h1 className="text-lg font-bold text-gray-800 mb-1">{docType}</h1>
            ...
            const petsOrPatients = freshUser.type === 'medico' ? (freshUser.patients || []) : (freshUser.pets || []);
            ...
            const resetDuenoState = () => { setSelectedPet(null); setDuenoView('main'); setShowEditProfile(false); setSelectedPaqueteDetail(null); setViewingOrderTracking(null); setCurrentExamForPet(null); setPendingExams([]); setCart([]); setShowNewPetForm(false); setShowAddressSelection(false); setShowNewAddressForm(false); setSelectedPetForOrder(null); setShowTomaMuestraStep(false); setPendingOrderAddress(null); };
                    ...
                    <div className="min-h-screen bg-gray-100">
                        ...
                        <div className="md:hidden">
                            ...
                            <div className="mob-header">
                                <div className="mob-header-inner">
                                    <div className="mob-header-logo cursor-pointer" onClick={() => { setMedicoView('inicio'); setSelectedMascota(null); setMenuOpen(false); setSelectedExam(null); setViewingInvoice(null); setCurrentExamForPet(null); setPendingExams([]); setCart([]); setShowNewPetForm(false); setShowAddressSelection(false); setShowNewAddressForm(false); setSelectedPetForOrder(null); setShowEditProfile(false); setViewingOrderTracking(null); }}>
                                        <img src="assets/logo_inulaboratorios.jpg" alt="INULAB" className="h-8" onError={(e) => { e.target.style.display='none'; e.target.nextSibling.style.display='flex'; }} />
                                        <div className="hidden w-8 h-8 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-lg items-center justify-center text-white font-bold text-sm">I</div>
                                    </div>
                                    <span className="mob-header-title">{freshUser.name}</span>
                                ...
                                <div className="mob-menu-overlay" onClick={() => setMenuOpen(false)}>
                                    <div className="mob-menu-panel" onClick={(e) => e.stopPropagation()}>
                                        ...
                                        <div className="px-4 py-3 space-y-1">
                                            ...
                                            <button onClick={() => { setMedicoView('facturacion'); setMenuOpen(false); setSelectedMascota(null); setSelectedExam(null); setViewingInvoice(null); setCurrentExamForPet(null); setPendingExams([]); setCart([]); setShowNewPetForm(false); setShowAddressSelection(false); setShowNewAddressForm(false); setSelectedPetForOrder(null); setShowEditProfile(false); setViewingOrderTracking(null); }} className="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl hover:bg-gray-50 text-gray-700 transition-colors">
                                                <div className="w-9 h-9 bg-amber-100 rounded-xl flex items-center justify-center"><i className="fas fa-file-invoice-dollar text-amber-600"></i></div>
                                        ...
                                        <div className="px-4 pt-4 pb-6 space-y-3">
                                            <div className="flex items-center gap-3 mb-1">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="text-gray-500 text-xs truncate">{getPetFullName(trackingOrder.items[0]?.pet)} · {trackingOrder.items[0]?.exam.name}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                                                ...
                                                <div className="space-y-2">
                                                    {trackingOrder.items.map((item, idx) => (
                                                        <div key={idx} className="flex items-center gap-3 p-2.5 bg-gray-50 rounded-xl">
                                                            <div className={`w-9 h-9 ${item.exam.bg} rounded-xl flex items-center justify-center flex-shrink-0`}>
                                                                <i className={`fas ${item.exam.icon} ${item.exam.color} text-sm`}></i>
                                                            </div>
                                                            <div className="flex-1 min-w-0">
                                                                <p className="font-medium text-gray-800 text-sm leading-tight truncate">{item.exam.name}</p>
                                                                <p className="text-gray-500 text-xs">{item.pet.photo} {getPetFullName(item.pet)}</p>
                                                            </div>
                                                        </div>
                                                    ))}
                                                ...
                                                <div className="mt-3 pt-3 border-t border-gray-100">
                                                    <div className="flex items-start gap-2 text-sm">
                                                        ...
                                                        <p className="text-gray-600 text-xs leading-snug">{trackingOrder.items[0]?.address?.address || 'Sin dirección'}, {trackingOrder.items[0]?.address?.district || ''}</p>
                                                    </div>
                                                </div>
                            ...
                            {/* Vista PDF móvil para médicos */}
                            {selectedExam && (
                                <div className="-mx-4 -mt-5">
                                    <div className="bg-white rounded-2xl shadow-lg p-4 m-4">
                                        <div className="flex items-center gap-3 mb-4">
                                            <button onClick={() => setSelectedExam(null)} className="w-10 h-10 bg-gray-100 hover:bg-gray-200 rounded-full flex items-center justify-center text-gray-600">
                                                <i className="fas fa-arrow-left"></i>
                                            </button>
                                            <div>
                                                <h1 className="text-lg font-bold text-gray-800">{selectedExam.type}</h1>
                                                <p className="text-gray-500 text-sm">
                                        ...
                                        {selectedExam.pdfData ? (
                                            <>
                                                <iframe src={selectedExam.pdfData} className="w-full rounded-xl border border-gray-200" style={{ height: 'calc(100vh - 250px)' }} title="PDF" />
                                                <div className="mt-3 flex justify-center">
                            ...
                            {/* Vista Comprobante móvil para médicos */}
                            {!selectedExam && viewingInvoice && (
                                <div className="-mx-4 -mt-5">
                                    <div className="bg-white rounded-2xl shadow-lg p-4 m-4">
                                        <div className="flex items-center gap-3 mb-4">
                                            <button onClick={() => setViewingInvoice(null)} className="w-10 h-10 bg-gray-100 hover:bg-gray-200 rounded-full flex items-center justify-center text-gray-600">
                                                <i className="fas fa-arrow-left"></i>
                                            </button>
                                            <div>
                                                <h1 className="text-lg font-bold text-gray-800">{viewingInvoice.documentType === 'boleta' ? 'Boleta' : 'Factura'}</h1>
                                                <p className="text-gray-500 text-sm">
                                                    {new Date(viewingInvoice.completedAt || viewingInvoice.date).toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' })}
                                                </p>
                                            </div>
                                        ...
                                        {viewingInvoice.invoicePdf ? (
                                            <>
                                                <iframe src={viewingInvoice.invoicePdf} className="w-full rounded-xl border border-gray-200" style={{ height: 'calc(100vh - 280px)' }} title="Comprobante PDF" />
                                                <div className="mt-3 flex justify-center">
                                                    <button onClick={() => { const link = document.createElement('a'); link.href = viewingInvoice.invoicePdf; link.download = `${viewingInvoice.documentType === 'boleta' ? 'Boleta' : 'Factura'}_${viewingInvoice.id}.pdf`; document.body.appendChild(link); link.click(); document.body.removeChild(link); }}
                                                        className={`flex items-center gap-2 px-6 py-2.5 ${viewingInvoice.documentType === 'boleta' ? 'bg-gradient-to-r from-cyan-500 to-blue-600' : 'bg-gradient-to-r from-amber-500 to-orange-600'} text-white rounded-xl font-semibold text-sm`}>
                                                        <i className="fas fa-download"></i> Descargar PDF
                                                    </button>
                                                </div>
                                            </>
                                        ...
                                        <div style={{position:'fixed', top:'calc(56px + env(safe-area-inset-top,0px))', left:0, right:0, bottom:'calc(64px + env(safe-area-inset-bottom,0px))', display:'flex', flexDirection:'column', background:'#f3f4f6', zIndex:50}}>
                                            ...
                                            <div style={{flex:1, overflowY:'auto', padding:'12px 16px'}}>
                                                {cart.map(item => (
                                                    <div key={item.exam.id} className="bg-white rounded-xl p-3 shadow-sm border border-gray-100 mb-2 flex items-center gap-3">
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            <p className="font-semibold text-gray-800 text-sm leading-tight truncate">{item.exam.name}</p>
                                            ...
                                            <div style={{background:'white', boxShadow:'0 -4px 16px rgba(0,0,0,0.10)', borderTop:'1px solid #e5e7eb', position:'relative', zIndex:2}}>
                                                <div className="px-4 pt-3 pb-3 space-y-2.5">
                                                    ...
                                                    {/* Botón solicitar */}
                                                    <button onClick={submitOrder} disabled={!termsAccepted} className={`w-full py-2.5 rounded-xl font-semibold text-sm ${termsAccepted ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-md' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
                        ...
                        <div className="hidden md:block min-h-screen">
                            ...
                            <div className="bg-white border-b border-gray-200 px-6 py-3 fixed top-0 left-0 right-0 z-50">
                                <div className="flex items-center justify-between">
                                    {/* Logo a la izquierda */}
                                    <div className="flex items-center gap-2 cursor-pointer ml-[-10px]" onClick={() => { setMedicoView('inicio'); setSelectedMascota(null); setSelectedExam(null); setViewingInvoice(null); setCurrentExamForPet(null); setPendingExams([]); setCart([]); setShowNewPetForm(false); setShowAddressSelection(false); setShowNewAddressForm(false); setSelectedPetForOrder(null); setShowEditProfile(false); setViewingOrderTracking(null); }}>
                                        <img src="assets/logo_inulaboratorios.jpg" alt="Logo" className="h-8" onError={(e) => { e.target.style.display = 'none'; }} />
                                        <span className="text-base font-light text-gray-700 tracking-wide">INULABORATORIOS</span>
                                    </div>
                            ...
                            <div className="pt-16 flex">
                                ...
                                <div className="w-56 bg-white border-r border-gray-200 fixed left-0 top-16 bottom-0 flex flex-col">
                                    ...
                                    <nav className="flex-1 p-3 space-y-2">
                                        ...
                                        <button
                                            onClick={() => { setMedicoView('pedidos'); setSelectedMascota(null); setSelectedExam(null); setViewingInvoice(null); setCurrentExamForPet(null); setPendingExams([]); setCart([]); setShowNewPetForm(false); setShowAddressSelection(false); setShowNewAddressForm(false); setSelectedPetForOrder(null); setShowEditProfile(false); setViewingOrderTracking(null); }}
                                        ...
                                        <button
                                            onClick={() => { setMedicoView('mascotas'); setSelectedMascota(null); setSelectedExam(null); setViewingInvoice(null); setCurrentExamForPet(null); setPendingExams([]); setCart([]); setShowNewPetForm(false); setShowAddressSelection(false); setShowNewAddressForm(false); setSelectedPetForOrder(null); setShowEditProfile(false); setViewingOrderTracking(null); }}
                                        ...
                                        <button
                                            onClick={() => { setMedicoView('resultados'); setSelectedMascota(null); setSelectedExam(null); setViewingInvoice(null); setCurrentExamForPet(null); setPendingExams([]); setCart([]); setShowNewPetForm(false); setShowAddressSelection(false); setShowNewAddressForm(false); setSelectedPetForOrder(null); setShowEditProfile(false); setViewingOrderTracking(null); }}
                                        ...
                                        <button
                                            onClick={() => { setMedicoView('facturacion'); setSelectedMascota(null); setSelectedExam(null); setViewingInvoice(null); setCurrentExamForPet(null); setPendingExams([]); setCart([]); setShowNewPetForm(false); setShowAddressSelection(false); setShowNewAddressForm(false); setSelectedPetForOrder(null); setShowEditProfile(false); setViewingOrderTracking(null); }}
                                ...
                                <div className="ml-56 flex-1 p-6 bg-gray-100 h-[calc(100vh-64px)] overflow-hidden">
                                            ...
                                            <div className="flex flex-col h-full -mx-6 -mt-6">
                                                ...
                                                <div className="flex-1 p-6 overflow-hidden">
                                                    <div className="flex gap-6 h-full">
                                                        {/* Columna izquierda - Timeline */}
                                                        <div className="w-1/2 bg-white rounded-2xl shadow-lg p-5 flex flex-col">
                                                            <h3 className="font-bold text-gray-800 mb-4">Estado del pedido</h3>
                                                                    ...
                                                                    return (
                                                                        <div key={step.id} className="flex gap-2">
                                                                            <div className="flex flex-col items-center">
                                                                                ...
                                                                                {!isLast && (
                                                                                    <div className={`w-0.5 flex-1 ${index < trackingCurrentIndex ? 'bg-cyan-500' : 'bg-gray-200'}`}></div>
                                                                                )}
                                                                            </div>
                                                                            <div className="flex-1">
                                                                                <p className={`font-medium text-sm ${isCompleted ? 'text-gray-800' : 'text-gray-400'}`}>{step.label}</p>
                                    ...
                                    {!selectedExam && !viewingInvoice && !currentExamForPet && !viewingOrderTracking && medicoView === 'inicio' && (
                                        <div className="h-full overflow-y-auto -mx-6 -mt-6 px-6 pt-6">
                                            ...
                                            <div className="mb-6">
                                                <div className="flex items-center justify-between mb-3">
                                                    ...
                                                    <button onClick={() => setMedicoView('pedidos')} className="text-cyan-600 text-sm font-medium hover:underline">Ver todos →</button>
                                                </div>
                                                {pendingOrders.length === 0 ? (
                                                    <div className="bg-white rounded-xl p-6 text-center shadow">
                                                        <i className="fas fa-check-circle text-3xl text-emerald-400 mb-2"></i>
                                                        <p className="text-gray-500">No existen pendientes</p>
                                                    </div>
                                                ) : (
                                                    <div className="grid grid-cols-2 gap-4">
                                                            ...
                                                            >
                                                                {order.items.map((item, idx) => (
                                                                    <div key={idx} className="flex items-center gap-3">
                                                                        <span className="text-3xl">{item.pet.photo}</span>
                                                                        <div className="flex-1">
                                                                            <p className="font-semibold text-gray-800">{getPetFullName(item.pet)}</p>
                                                                            <p className="text-sm text-gray-500">{item.exam.name}</p>
                                                                        </div>
                                                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all group-hover:scale-110 ${order.status === 'moto_arrived' || order.status === 'results_uploaded' ? 'bg-red-500' : 'bg-cyan-500'}`}>
                                                                            <i className="fas fa-route text-white"></i>
                                                                        </div>
                                                                    </div>
                                                                ))}
                                                                {order.status === 'moto_arrived' && (
                                                                    <div className="mt-3 bg-red-50 border border-red-300 rounded-lg p-2 flex items-center gap-2 animate-pulse">
                                                                        <i className="fas fa-map-marker-alt text-red-600"></i>
                                                                        <p className="text-red-800 font-medium text-xs">¡El motorizado llegó! Entrega la muestra</p>
                                                                    </div>
                                                                )}
                                                                {order.status === 'results_uploaded' && (
                                                                    <div className="mt-3 bg-red-50 border border-red-300 rounded-lg p-2 flex items-center gap-2 animate-pulse">
                                                                        <i className="fas fa-credit-card text-red-600"></i>
                                                                        <p className="text-red-800 font-medium text-xs">¡Resultados listos! Pendiente de pago</p>
                                                                    </div>
                                                                )}
                                                            </div>
                                                        ))}
                                            ...
                                            <div className="mb-6">
                                                <div className="flex items-center justify-between mb-3">
                                                    <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                                                        <i className="fas fa-file-medical text-purple-500"></i> Último resultado
                                                        {newResultHighlight && <span className="bg-emerald-500 text-white text-xs px-2 py-0.5 rounded-full animate-pulse">¡NUEVO!</span>}
                                                    </h2>
                                                ...
                                                {completedOrders.length === 0 ? (
                                                    ...
                                                    return (
                                                        <div className={`bg-white rounded-xl p-4 shadow hover:shadow-lg transition-all cursor-pointer ${newResultHighlight ? 'highlight-green-pulse' : ''}`} onClick={() => exam?.pdfData && setSelectedExam(exam)}>
                                                            <div className="flex items-center gap-4">
                                                                <span className="text-3xl">{lastOrder.items[0].pet.photo}</span>
                                                                <div className="flex-1">
                                                                    <p className="font-medium text-gray-800">{getPetFullName(lastOrder.items[0].pet)}</p>
                                                                    <p className="text-sm text-gray-500">{lastOrder.items[0].exam.name}</p>
                                                                    <p className="text-xs text-gray-400">{new Date(lastOrder.completedAt).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                                                                </div>
                                            ...
                                            <div>
                                                <div className="flex items-center justify-between mb-3">
                                                    <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                                                        <i className="fas fa-file-invoice-dollar text-amber-500"></i> Último comprobante
                                                        {newInvoiceHighlight && <span className="bg-emerald-500 text-white text-xs px-2 py-0.5 rounded-full animate-pulse">¡NUEVO!</span>}
                                                    </h2>
                                                    ...
                                                    if (invoicesWithPdf.length === 0) {
                                                        return (
                                                            <div className="bg-white rounded-xl p-6 text-center shadow">
                                                                ...
                                                                <p className="text-gray-500">No hay comprobantes aún</p>
                                                            </div>
                                                        );
                                                    }
                                                    const lastInvoice = invoicesWithPdf[0];
                                                    return (
                                                        <div className={`bg-white rounded-xl p-4 shadow hover:shadow-lg transition-all cursor-pointer ${newInvoiceHighlight ? 'highlight-green-pulse' : ''}`} onClick={() => setViewingInvoice(lastInvoice)}>
                                                            <div className="flex items-center gap-4">
                                                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${newInvoiceHighlight ? 'bg-emerald-100' : lastInvoice.documentType === 'boleta' ? 'bg-cyan-100' : 'bg-amber-100'}`}>
                                                                    <i className={`fas ${lastInvoice.documentType === 'boleta' ? 'fa-receipt' : 'fa-file-invoice'} text-xl ${newInvoiceHighlight ? 'text-emerald-600' : lastInvoice.documentType === 'boleta' ? 'text-cyan-600' : 'text-amber-600'}`}></i>
                                                                </div>
                                                                <div className="flex-1">
                                                                    <p className="font-medium text-gray-800">{lastInvoice.documentType === 'boleta' ? 'Boleta' : 'Factura'}</p>
                                                                    <p className="text-sm text-gray-500">{getPetFullName(lastInvoice.items[0].pet)} - {lastInvoice.items[0].exam.name}</p>
                                                                    <p className="text-xs text-gray-400">{new Date(lastInvoice.completedAt).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                                                                </div>
                                                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${newInvoiceHighlight ? 'bg-emerald-100' : 'bg-amber-100'}`}>
                                                                    <i className={`fas fa-eye ${newInvoiceHighlight ? 'text-emerald-600' : 'text-amber-600'}`}></i>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    );
            ...
            return (
                <div className="min-h-screen bg-gray-100">
                    ...
                    <div className="app-content-dueno lg:ml-56">
                                    ...
                                    <>
                                        {showTomaMuestraStep && pendingOrderAddress ? (
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                {userAddresses.map((addr, idx) => (<div key={idx} onClick={() => handleDuenoAddressSelect(addr)} className="bg-white rounded-2xl p-4 shadow cursor-pointer hover:shadow-lg hover:border-cyan-500 border-2 border-transparent transition-all"><div className="flex items-center gap-3"><div className="w-10 h-10 bg-cyan-100 rounded-xl flex items-center justify-center"><i className="fas fa-map-marker-alt text-cyan-600"></i></div><div className="flex-1"><p className="font-bold text-gray-800">{addr.name}</p><p className="text-gray-500 text-sm">{addr.address}</p><p className="text-gray-400 text-xs">{addr.district} • Ref: {addr.reference}</p></div><i className="fas fa-chevron-right text-gray-400"></i></div></div>))}
                                                <button onClick={() => setShowNewAddressForm(true)} className="w-full py-3 rounded-xl border-2 border-dashed border-gray-300 text-gray-500 hover:border-cyan-400 hover:text-cyan-600 transition-all"><i className="fas fa-plus mr-2"></i>Agregar nueva dirección</button>
                                            </div>
                                        ) : (
                                            <div className="bg-white rounded-2xl shadow p-6">
                        ...
                        {!viewingOrderTracking && !(selectedPet && activeTab === 'resultados') && !currentExamForPet && (<>
                        {activeTab === 'pedidos' && (<div>
                                                ...
                                                {userOrders.map(order => {
                                                    ...
                                                    return (
                                                        <div key={order.id} className="bg-white rounded-2xl shadow p-4">
                                                            <div className="flex items-center justify-between mb-3">
                                                                <p className="text-xs text-gray-500">
                                                                    {new Date(order.date).toLocaleDateString('es-ES', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                                                                </p>
                                                                <button
                                                                    onClick={() => setViewingOrderTracking(order)}
                                                            ...
                                                            <div className="space-y-2">
                                                                {order.items.map((item, idx) => (
                                                                    <div key={idx} className="flex items-center gap-3 p-2 bg-gray-50 rounded-xl">
                                                                        <div className={`w-9 h-9 ${item.exam.bg} rounded-lg flex items-center justify-center`}>
                                                                            <i className={`fas ${item.exam.icon} ${item.exam.color} text-sm`}></i>
                                                                        </div>
                                                                        <div className="flex-1 min-w-0">
                                                                            <p className="text-gray-800 font-medium text-sm truncate">{item.exam.name}</p>
                                                                            <p className="text-gray-500 text-xs">{item.pet.photo} {getPetFullName(item.pet)}</p>
                                                                        </div>
                                                                    </div>
                                                                ))}
